...
var THRESHOLDS = { safe_margin: 0.008, balanced_low: -0.005, reach_low: -0.020 };

var FIELD_KEYWORDS = {
  "경영": ["경영", "회계", "비즈니스"],
  "경제": ["경제", "금융", "재무"],
  "심리": ["심리", "상담"],
  "공학": ["공학", "기계", "전기", "전자", "토목", "건축", "화공", "신소재", "재료"],
  ...
  "약학": ["약학"],
  "교육": ["교육", "교직"],
  "어문": ["국어국문", "영어영문", "중어중문", "일어일문", "독어독문", "불어불문"],
  "사회": ["사회", "행정", "정치", "외교", "언론"],
  "기계공학": ["기계"]
};
...
function computeStudentScore(student, formula) {
  ...
  var eng_contrib = (w["영어"] > 0 && eng_raw >= 10) ?
    eng_raw * (w["영어"] / 100) * (max_score / 100) : eng_raw;
  var hist_contrib = 0;
  if (formula.korean_history_grade_table && sc["한국사"]) {
    hist_contrib = formula.korean_history_grade_table[sc["한국사"].grade - 1] || 0;
  }
  return {
...
function generateScenarios(student, data) {
  ...
  var valid_formulas = formulas.filter(function (f) {
    ...
    if (student.track === "자연" && f.track.indexOf("인문") !== -1) return false;
    if (student.track === "인문" && (f.track.indexOf("자연") !== -1 || f.track.indexOf("이공") !== -1)) return false;
    if (!f.source) return false;
    var src = String(f.source);
    if (src.indexOf("잠정") !== -1 || src.indexOf("검증필요") !== -1) return false;
    return true;
  });
  ...
  for (var fi = 0; fi < valid_formulas.length; fi++) {
    ...
    var cut_values = dept_cuts_all.map(function (c) { return c.cut70; })
      .filter(function (v) { return typeof v === "number" && v > 50; });
    if (cut_values.length === 0) continue;
    var cut_sorted = cut_values.slice().sort(function (a, b) { return a - b; });
    var cut_max = cut_sorted[cut_sorted.length - 1];
    var student_scaled = Math.round(sc.total / sc.max_score * cut_max / 0.95 * 10) / 10;

    for (var di = 0; di < dept_cuts_all.length; di++) {
      var dept = dept_cuts_all[di];
      if (!isValidDept(dept.dept)) continue;
      // 이상 데이터 제외
      if (dept.dept === "모집단위" || dept.dept === "모집인원" ||
          dept.dept.indexOf("모집단위") === 0 || dept.dept.indexOf("모집인원") === 0) continue;
      ...
      results[bucket].push({
        univ: formula.univ, univ_short: norm_univ, campus: formula.campus,
        dept: dept.dept, group: dept.group, capacity: dept.capacity,
...
function pickRecommendation(s) {
  return { safe: s.safe.slice(0, 5), balanced: s.balanced.slice(0, 5), reach: s.reach.slice(0, 5) };
}
...
function generateBalancedRationale(p) {
  ...
  return p.univ + " " + p.dept + " (" + (p.group || "정시") + "): 작년 70cut " + p.cut70 +
    " vs 학생 환산 " + p.student_score + " (" + m + "). 만점 대비 " + cr + "%. 모집 " +
    p.capacity + "명, 작년 경쟁률 " + p.competition + "배.";
}

function generateRisks(scenarios, student) {
  var risks = [];
  if (scenarios.balanced.length === 0) risks.push("균형권 학과 없음");
  if (scenarios.safe.length < 2) risks.push("안전권 부족 — 추가 안전권 확보 필요");
  if (scenarios.reach.length === 0 && scenarios.balanced.length > 0) risks.push("도전권 없음 — 보수적 포트폴리오");
  if (student.scores["영어"] && student.scores["영어"].grade >= 4) risks.push("영어 " + student.scores["영어"].grade + "등급 — 감점 큰 대학 불리");
  if (student.scores["한국사"] && student.scores["한국사"].grade >= 5) risks.push("한국사 " + student.scores["한국사"].grade + "등급 — 일부 대학 감점");
  risks.push("INFO: 정시 환산공식 4개 검증 (연세대·가천대). 잠정 28개 제외 중. 7월 모집요강 후 확장.");
  return risks;
}

function generateParentSummary(picks, student) {
  var b = picks.balanced[0], s = picks.safe[0], r = picks.reach[0];
  return {
    student_name: student.name, season_label: "2027학년도 정시 (검증 4개대)",
    headline: b ? (student.name + " 학생, 정시 균형권: " + b.univ + " " + b.dept) : (student.name + " 학생 — 정시 균형권 미확보"),
    three_options: {
    ...
    this_week_do: ["6월 모의고사 응시 (백분위 확정)", "균형권 학과 모집요강 출력", "원서접수 일정 캘린더 등록"],
    this_week_dont: ["타 학원/유튜브 정시 컨설팅 병행 — 혼선", "지금 환산점수만으로 지원 확정"],
...
function generateDirectorScript(picks, student) {
  ...
  return [
    student.name + " 학생, " + b.univ + " " + b.dept + " 작년 70cut " + b.cut70 + ", 학생 환산 " + b.student_score + " (차이 " + d + ").",
    "작년 경쟁률 " + b.competition + "배, 모집 " + b.capacity + "명. 합격 약 50% 구간.",
...
function generateSusiScenarios(student, data) {
  var universities = data.universities, susi_cuts = data.susi_cutoffs;
  var student_grade = student["내신"];
  if (typeof student_grade !== "number") return { safe: [], balanced: [], reach: [], fail: [] };
  ...
  for (var i = 0; i < susi_cuts.length; i++) {
    ...
    for (var sk = 0; sk < SPECIAL.length; sk++) {
      if (tname.indexOf(SPECIAL[sk]) !== -1) { skip = true; break; }
    }
    if (skip) continue;
    ...
    results[bucket].push({
      univ: c.univ, univ_short: normalizeUniv(c.univ), campus: c.campus,
...
function generateSusiRationale(p) {
  ...
  var m = p.diff >= 0 ? "+" + p.diff : String(p.diff);
  return p.univ + " " + p.dept + " (" + (p["전형분류"] || "수시") + " - " + (p["전형"] || "") +
    "): 작년 70cut " + p.cut70_grade + " vs 학생 내신 " + p.student_grade + " (차이 " + m + "). 모집 " +
    p.capacity + "명, 경쟁률 " + p.competition + "배.";
}

function generateCombinedParentSummary(jp, sp, student) {
  var jB = jp ? jp.balanced[0] : null, sB = sp ? sp.balanced[0] : null;
  var headline = sB ? student.name + " 수시 균형: " + sB.univ + " " + sB.dept :
    (jB ? student.name + " 정시 균형: " + jB.univ + " " + jB.dept :
      student.name + " — 균형권 미확보");
  return {
    student_name: student.name, season_label: "2027학년도 수시 + 정시 (검증 4개대)",
    headline: headline,
    susi_picks: sp ? {
      safe: sp.safe[0] ? { name: sp.safe[0].univ + " " + sp.safe[0].dept, type: sp.safe[0]["전형분류"], prob: "약 80%" } : null,
      balanced: sB ? { name: sB.univ + " " + sB.dept, type: sB["전형분류"], prob: "약 50%" } : null,
      reach: sp.reach[0] ? { name: sp.reach[0].univ + " " + sp.reach[0].dept, type: sp.reach[0]["전형분류"], prob: "약 20%" } : null
    } : null,
    jeongsi_picks: jp ? {
      safe: jp.safe[0] ? { name: jp.safe[0].univ + " " + jp.safe[0].dept, prob: "약 80%" } : null,
      balanced: jB ? { name: jB.univ + " " + jB.dept, prob: "약 50%" } : null,
      reach: jp.reach[0] ? { name: jp.reach[0].univ + " " + jp.reach[0].dept, prob: "약 20%" } : null
    } : null,
    this_week_do: ["수시 6장 모집요강 출력", "9월 모평 응시", "학생부 마감 (8월 31일) 전 보완"],
...
function generateCombinedDirectorScript(jp, sp, student) {
  var sB = sp ? sp.balanced[0] : null, sS = sp ? sp.safe[0] : null, jB = jp ? jp.balanced[0] : null;
  if (sB) {
    var d = sB.diff >= 0 ? "+" + sB.diff : String(sB.diff);
    return [
      student.name + " 학생 수시 균형: " + sB.univ + " " + sB.dept + " (" + sB["전형분류"] + "). 작년 70cut " + sB.cut70_grade + ", 학생 " + sB.student_grade + " (차이 " + d + ").",
      sS ? "안전권은 " + sS.univ + " " + sS.dept + " 우선 검토. 정시 백업은 " + (jB ? jB.univ + " " + jB.dept : "9월 모평 후 확정") + "." : "안전권 1-2장 추가 발굴 필요.",
      "9월 모평 직후 수능최저 충족 가능성 재확인 + 수시 6장 최종 확정."
    ];
  }
  return [
    student.name + " 학생, 현재 내신(" + student["내신"] + ")로 수시 균형권 어려움.",
    "희망 학과/지역 완화 또는 종합전형 (비교과 강점) 전환 검토.",
    jB ? "정시 균형 " + jB.univ + " " + jB.dept + " 잡힘 — 정시 비중 강화 가능." : "9월 모평 결과로 정시 백업 확정."
  ];
}
...
var __api = {
  ...
  classifyTransType: classifyTransType, classifySusi: classifySusi,
  normalizeUniv: normalizeUniv, normalizeCampus: normalizeCampus,
  isValidDept: isValidDept
};

if (typeof window !== "undefined") { window.ScenarioEngine = __api; }
