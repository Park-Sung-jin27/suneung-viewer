...
- **ok:false(부적절) 선지 해설 = 3단 대조 서술 의무 ([Adopted 2026-06-22 대표 결정])**: 정답은 "근거 있다→납득"(단방향)이지만 오답은 "선지는 X라는데 지문은 Y"(대조)라야 이해됨. 정답 3단과 대칭으로 의무화. ① 🔍 **선지 분해**(선지 주장을 배경 / 주체·관점 / 핵심 주장 조각으로) → ② 📌 **지문 대조**(근거 문장 verbatim 인용 — 선지가 *맞게* 짚은 부분 인정 + 지문이 *실제로* 말하는 바) → ③ ❌ **어긋난 지점 + pat 명명**(선지는 X, 지문은 Y → 무엇이 뒤바뀌었나). 타깃(3~4등급) 눈높이. 📌 근거는 sent.t verbatim(§13⑥ 게이트 C_anchor 통과 의무). **precedent: r20276b Q5②**(㉠이 형상화한 주체를 노비↔양반 뒤바꿈 = R2 관계 전환 — 대표 모델 해설).
