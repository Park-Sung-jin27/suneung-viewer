...
[ok의 정의 — 오해 금지]
ok는 '정답 여부'가 아니라 '선지 진술이 지문에 비추어 적절한가'다.
· 부정발문(적절하지 않은 것은) → ok:false 선지가 정답이다.
· 긍정발문(적절한 것은)       → ok:true  선지가 정답이다.
[정답 정보]의 정답 번호와 [선지 판정 — 정본(절대)]의 ok는 항상 이 규칙으로 정합한다.
둘이 어긋나 보이면 네 해석이 틀린 것이다. ok를 절대 기준으로 삼아라.
- ok:true 선지(진술이 적절): pat = null.
  🔍는 이 선지가 왜 적절한지를 논증한다. 결론줄은 반드시 ✅로 시작한다.
- ok:false 선지(진술이 부적절): pat = 독서 R1~R4 / 문학 L1~L5 / 어휘 V 중 하나.
  🔍는 왜 부적절한지를 논증한다. 결론줄은 반드시 ❌로 시작한다.
- 결론줄은 analysis의 **마지막 줄에 단독으로** 두고 ✅ 또는 ❌로 시작할 것.
