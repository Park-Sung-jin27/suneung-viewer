      ...
      for (const s of yd[sec]) {
        ...
        for (const sent of (s.sents || [])) {
          if (sent.type === 'image' || sent.sentType === 'image') {
            ...
            if (u) {
              const fname = u.startsWith('/images/') ? u.slice('/images/'.length) : u.replace(/^\//,'');
              referenced.add(fname);
              const abs = path.join(IMG_DIR, fname);
              if (!fs.existsSync(abs)) {
                findings.push({ yk, setId: s.id, sec, issue: 'K_IMG_MISSING', url: u, sentId: sent.id });
                sum.K += 1;
              }
            } else {
              findings.push({ yk, setId: s.id, sec, issue: 'K_IMG_MISSING', url: null, sentId: sent.id });
              sum.K += 1;
            }
          }
        }
...
function main() {
  ...
  console.log(`image sents: ${sum.image_sents}  hasFig sets: ${sum.hasFig_sets}`);
  console.log(`K img missing: ${sum.K}  L hasFig-no-img: ${sum.L}  M orphan: ${sum.M}`);
  console.log('');
