...
const VERIFIED = {
  ...
  // ── D-110-2 ② 수리 ─ bracket_map_v2 역방향 매핑 + 경계 문장 y 직접 대조 ──
  r2019d:  { by: "엔지니어(벡터)", commit: "bba0793", note: "10면 좌단 823.3/1048.4 → s13~s18. 기존 s16~s18 은 시작 3문장 늦음" },
  r20209a: { by: "엔지니어(벡터)", commit: "4559d27", note: "8면 우단 419.5/587.0 → s37~s41. s37 y=416.1(−3.4)" },
  r20209b: { by: "엔지니어(벡터)", commit: "4e9b84c", note: "10면 좌단 270.3/437.8 → s4~s10" },
  l2020a:  { by: "엔지니어(벡터)", commit: "1194cb2", note: "8면 214.1/401.5 → s2~s12. 기존 s5~s12" },
  l20249d: { by: "엔지니어(벡터)", commit: "fa6686b", note: "11면 우단 383.2/696.6 → s11~s28. 기존 s19~s31 은 전혀 다른 범위" },
  l20259a: { by: "엔지니어(벡터)", commit: "6fa1456", note: "6면 우단 199.3/389.9 → s2~s12. ann(s6~s12)과 vm(s2~s4)이 달랐고 화면은 vm 을 그렸다 — ann+vm 동시 수정" },
  l20259c: { by: "엔지니어(벡터)", commit: "1129612", note: "10면 좌단 880.7/1057.2 → s41~s51. 기존 s20~s27. ann+vm 동시 수정" },
  r20229d: { by: "엔지니어(벡터)", commit: "fa68cbb", note: "5면 우단 420.4/733.8. 신규 정박 — ann·vm 어디에도 없었다" },
  l20229b: { by: "엔지니어(벡터)", commit: "df1212d", note: "8면 좌단 765.8/839.5. 신규 정박 [A]. [B] 는 아직 미정박" },
  // l20219a 는 라벨별로 갈린다 — [A] 는 화면값이 맞았고 [B] 만 고쳤다.
  "l20219a::A": { by: "엔지니어(벡터)", commit: "(수리 없음)",
    note: "🟢 C-6 오판. 6면 좌단 760.5/1019.0, 라벨 글리프 [A] y=885.2 정중앙 → s1~s2 = 화면값. v2 초기 판정 s13~s21 은 좌단 브래킷이 우단 문장을 먹은 도구 결함이었다" },
  "l20219a::B": { by: "엔지니어(벡터)", commit: "eb80e90", note: "6면 우단 273.0/624.1 → s6~s10. s6 y=269.0(−4.0)" },

  // ── D-110-2 ③ C-6 오판 확정 — 값 불변, 기록만 ──────────────────────
  //   C-7 기계 대조가 불일치로 지목했으나 벡터 판독 결과 화면값이 맞았다.
  l20169d: { by: "엔지니어(벡터)", commit: "(수리 없음)", note: "🟢 C-6 오판. 16면 196.1/219.2 → s1~s2, 917.7/940.8 → s41~s42 둘 다 화면값과 같다. 라벨 없는 구간 2개(s7~s13·s30~s37)가 더 있으나 문항이 [A][B] 만 참조해 정박 보류" },
  l2019a:  { by: "엔지니어(벡터)", commit: "(수리 없음)", note: "🟢 C-6 오판. 12면 5구간(214.1/237.4·324.4/364.6·379.6/439.0·471.4/494.6·563.3/603.5)이 전부 화면값과 같다" },
  l2022a:  { by: "엔지니어(벡터)", commit: "(수리 없음)", note: "🟢 C-6 오판. 6면 5구간 전부 화면값과 같다. C-7 이 A·B·C·D 넷을 지목했으나 모두 일치" },
  l2024a:  { by: "엔지니어(벡터)", commit: "(수리 없음)", note: "🟢 C-6 오판. 6면 199.3/660.2 → s1~s5 = 화면값" },
  "l20196c::A": { by: "엔지니어(벡터)", commit: "(수리 없음)",
    note: "🟢 C-6 오판. 11면 좌단 893.2/1023.0. 마지막 포함 행 y=1016.7(−6.3)=s18 이고 <제2연>(s19) 은 y=1035.1 로 꺾쇠 밖이다 → s11~s18 = 화면값. v2 가 s19 를 넣은 것은 확장 규칙 오작동" },
  "l20196c::B": { by: "심사관", commit: "(작업 불필요)",
    note: "🟢 종결. 34번 문항 <보기> 안 한시 표지이지 본문 구간 표시가 아니다 — l2022c 유형" },

  // ── D-111 ① · D-112 ① 수리 ─────────────────────────────────────────
  r2023b:  { by: "엔지니어(벡터)", commit: "dffa579", note: "2면 좌단 꺾쇠 217.7/550.5 → s1~s8. ann 이 맞고 vm(s2~s9)이 틀려 vm 만 동기화. F-25 머지 차단 해소" },
  r20169g: { by: "엔지니어(벡터)", commit: "484c63e", note: "15면 우단 [A] 162.0/332.2 → s7 · [B] 621.0/754.4 → s16. 라벨이 각 구간 정중앙. 신규 정박" },
  l20229c: { by: "엔지니어(벡터)", commit: "2be9f5f", note: "10면 우단 [A] 744.4/764.8 → s19~s21 · [B] 854.7/875.1 → s26~s28. 신규 정박(비노출). l20229b [B] 귀속을 확인하다 찾았다" },
  "l2024c::C": { by: "엔지니어(벡터)", commit: "9401bec",
    note: "10면 우단 365.2/403.7. [B] 와 같은 문장이라 first-match 에 밀려 영영 미렌더였다(LIVE). s7 을 발화 경계에서 s7(195자)+s7b(469자)로 분리해 해소 — cs_spans 6·cs_ids 10·underline ㉤·vm 2건 동시 이관" },

  // ── D-112 유형 종결 ─ <보기>/기록의 자리 표시자. 본문 정박 대상이 아니다 ──
  l20259b: { by: "심사관", commit: "(작업 불필요)", note: "🟢 종결. 문항 <보기>의 「들어갈 내용」 자리 표시자. 본문 sents 에 [A~F] 0건 — l2022c 유형" },
  r20229c: { by: "심사관", commit: "(작업 불필요)", note: "🟢 종결. 위와 같음" },
  r2020e:  { by: "심사관", commit: "(작업 불필요)", note: "🟢 종결. 위와 같음" },
  r20176a: { by: "엔지니어(벡터)", commit: "(작업 불필요)",
    note: "🟢 종결. 7면 라벨 [A] y=906.1 · [B] y=1002.1 은 <보기> 안 조건 목록(「◦입력 단자는 세 개…」)의 항목 표지다. 그 x 대역에 브래킷 세로선이 없다 — l2022c 유형" },
  l20146a: { by: "엔지니어(벡터)+심사관(화면 실측)", commit: "(조치 불필요)",
    note: "🟢 정상. 13면 꺾쇠 197.5/292.2 → s1~s6 = 화면값. 심사관 화면 실측으로 s1~s6 렌더 확인" },

  // ── D-112 판독 불가 보류 ─ 2014 시험지는 텍스트층 없는 스캔본 ───────────
  l2014c:  { by: "—", commit: "(보류)", note: "⏸ 판독 불가. 2014수능A 시험지 PDF 에 텍스트층이 없다(문장 검색 전 지면 0건, 라벨 글리프 0개). OCR 은 §13⑬ 위반이라 쓰지 않는다" },
  l2014bB: { by: "—", commit: "(보류)", note: "⏸ 판독 불가. 2014수능B 시험지 — 위와 같음" },
  l2014dB: { by: "—", commit: "(보류)", note: "⏸ 판독 불가. 2014수능B 시험지 — 위와 같음" },

  // ── D-115 ③ 웨이브 1 배치 1 ────────────────────────────────────────
  r20266b: { by: "엔지니어(벡터)", commit: "1e7e2d2",
    note: "2면 좌단 꺾쇠 493.4/734.3, 라벨 [A] y=609.2 가 구간 중앙(613.9) → s9~s16. 신규 정박(비노출)" },
};
