...
const step3Path = process.argv[2] || path.resolve(__dirname, 'test_data/step3_result_2022.json');
const answerKeyPath = process.argv[3] || path.resolve(__dirname, 'test_data/answer_key_2022.json');
...
for (const set of allSets) {
  for (const q of set.questions) {
    ...
    if (!correctChoice) {
      console.warn(`  ⚠️  ${set.id} ${q.id}번: 정답 선지(${correctNum}번) 없음`);
      continue;
    }
...
if (mismatchList.length === 0) {
  ...
  for (const m of mismatchList) {
    console.log(`  - [${m.set}] ${m.qId}번 (${m.questionType}) 정답:${m.correctNum}번 → ok=${m.actualOk} (기대값: ${m.expectedOk})`);
  }
}
...
for (const targetId of ['r2022a', 'l2022a']) {
  ...
  if (!set) { console.log(`\n[${targetId}] 세트 없음`); continue; }

  const q = set.questions[0];
  console.log(`\n[${targetId}] ${q.id}번 문항: "${q.t}"`);
  console.log(`questionType: ${q.questionType}  /  정답: ${answerKey[String(q.id)]}번`);
  ...
  for (const q of set.questions) {
    for (const c of q.choices) {
      ...
      if (c.ok === true && c.pat !== null) {
        okTruePatNotNull++;
        okTruePatNotNullList.push(`[${set.id}] ${q.id}번 선지${c.num} (pat:${c.pat})`);
      }
    }
  }
}
