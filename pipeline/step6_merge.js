...
if (!step5Path || !examKey || !section) {
  console.error(
    "사용법: node pipeline/step6_merge.js <step5결과JSON> <시험키> <섹션>",
  );
  console.error(
    '예시: node pipeline/step6_merge.js pipeline/test_data/step5_result_2022.json "2022수능" reading',
  );
  process.exit(1);
}
    ...
    const mergedQuestions = newSet.questions.map((newQ) => {
      ...
      const mergedChoices = newQ.choices.map((newC) => {
        ...
        return {
          ...
          cs_ids:
            existingC.cs_ids && existingC.cs_ids.length > 0
              ? existingC.cs_ids
              : newC.cs_ids || [],
        };
      });
