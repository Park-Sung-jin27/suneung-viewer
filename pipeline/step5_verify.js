...
async function rederiveAnswers(set) {
  const questionBlocks = set.questions.map(q => {
    const choicesText = q.choices.map(c =>
      `  선지${c.num}: ${c.analysis || '(해설 없음)'}`
    ).join('\n');
    return `[${q.id}번]\n${choicesText}`;
  }).join('\n\n');
...
async function verifySetByReanalysis(set, answerKey) {
  ...
  for (const q of set.questions) {
    ...
    if (correct !== undefined && derivedAnswer !== undefined && correct !== derivedAnswer) {
...
export async function verifyAndFix(step4Data, answerKey, { step2Data, maxRetries = 3 } = {}) {
  const result = {
  ...
  for (const section of ['reading', 'literature']) {
    for (let si = 0; si < result[section].length; si++) {
      ...
      if (mismatches.length === 0) {
        ...
        console.log(`  ❌ 불일치 ${mismatches.length}건: ${mismatches.map(m => `${m.qId}번(기대:${m.expected} 재도출:${m.derived})`).join(', ')}`);

        if (!step2Data) {
          console.log(`  ⚠️  step2Data 없음 — 재실행 불가, needsReview 플래그`);
          mismatches.forEach(m => stats.needsReview.push({ setId: set.id, qId: m.qId }));
        } else {
          // 최대 3회 재시도
          let attempt = 0;
          while (mismatches.length > 0 && attempt < maxRetries) {
            ...
            // step2 원본 세트 찾기
            const step2Set = [...step2Data.reading, ...step2Data.literature]
              .find(s => s.id === set.id);
            if (!step2Set) {
            ...
            if (mismatches.length === 0) {
              ...
              console.log(`  ❌ retry ${attempt}: 불일치 ${mismatches.length}건 남음`);
            }
          }

          if (mismatches.length > 0) {
            console.log(`  ⚠️  ${maxRetries}회 후에도 불일치 — needsReview 플래그`);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (!step4Path || !answerKeyPath) {
    console.error('사용법: node pipeline/step5_verify.js [step4결과JSON] [정답키JSON]');
    process.exit(1);
  }
  ...
  const step2File = fs.readdirSync(dir).find(f => f.startsWith('step2_result'));
  let step2Data = null;
  if (step2File) {
  ...
  verifyAndFix(step4Data, answerKey, { step2Data })
    .then(({ result, stats }) => {
      ...
      console.log(`정답률: ${stats.matched}/${stats.total} (${Math.round(stats.matched / stats.total * 100)}%)`);
      if (stats.needsReview.length > 0) {
        console.log(`\nneedsReview 문항 (${stats.needsReview.length}건):`);
        stats.needsReview.forEach(r => console.log(`  - [${r.setId}] ${r.qId}번`));
      } else {
        console.log('needsReview 문항: 없음 ✅');
      }

      const outPath = path.resolve(dir, path.basename(step4Path_abs).replace('step4_', 'step5_'));
