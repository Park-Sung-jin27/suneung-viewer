...
const BETA = ["2026수능","2025수능","2024수능","2023수능","2022수능","2022_6월","2023_6월","2024_6월","2025_6월","2026_6월","2022_9월","2023_9월","2024_9월","2025_9월","2026_9월"];
...
function pdfLines(yearKey) {
  ...
  const txt = execFileSync("pdftotext", ["-layout", pdf, "-"], { maxBuffer: 64e6 }).toString("utf8");
  return txt.split("\n");
}
...
function assignSet(set, heads) {
  ...
  sents.forEach((s, i) => {
    ...
    if (s.sentType === "workTag" || /^\((가|나|다|라)\)/.test(s.t.trim())) { bounds.push(i); return; }
...
for (const yk of years) {
  ...
  for (const set of Y.reading || []) {
    ...
    if (totalParas < 2 || totalParas > 15 || avg > 11) {
      report.push(`  ⚠ ${yk} ${set.id}: 문단 ${totalParas}개 (sents ${sc}, 평균 ${avg.toFixed(1)}) — 비정상, skip (needs_human)`);
      skipped++;
      continue;
    }
    report.push(`  ${yk} ${set.id}: 문단 ${totalParas}개 / sents ${sc} (경계 sentIdx: ${bounds.join(",")})`);
    if (APPLY) {
      set.sents.forEach((s, i) => { s.para = assigned[i]; });
      applied++;
    }
  }
}
...
if (APPLY) {
  const bak = path.join(ROOT, "pipeline/backups", `all_data_204.before_para_${new Date().toISOString().slice(0, 10).replace(/-/g, "")}.json`);
