    ...
    for (const sec of ['reading', 'literature']) {
      ...
      for (const s of yd[sec]) {
        for (const sent of (s.sents || [])) {
          ...
          if (noUnder !== sent.id) idx.set(noUnder, { t: sent.t || '', yk, setId: s.id });
          const withUnder = sent.id.replace(/([a-z0-9]+)s(\d+)$/, '$1_s$2');
          if (withUnder !== sent.id) idx.set(withUnder, { t: sent.t || '', yk, setId: s.id });
        }
      }
    }
  }
...
function audit() {
  ...
  function rec(yk, setId, sec, range) {
    const k = `${yk}/${setId}`;
    if (!records.has(k)) {
      records.set(k, { yk, setId, sec, range: range || '?', C: 0, E: 0, F: 0, G: 0, L: 0, grading: 0, total_q: 0, total_sent: 0, hasFig: false });
    }
      ...
      for (const s of yd[sec]) {
        ...
        if (s.range && r.total_q > 0) {
          ...
          if (m) {
            ...
            const exp = []; for (let i = lo; i <= hi; i++) exp.push(i);
            const act = (s.questions || []).map(q => q.id).filter(x => Number.isInteger(x)).sort((a, b) => a - b);
            const same = act.length === exp.length && act.every((v, i) => v === exp[i]);
            if (!same) r.E += 1;
          }
        }
      }
  ...
  for (const r of records.values()) {
    if (r.hasFig) {
      ...
      const hasImgSent = (s?.sents || []).some(x => x.type === 'image' || x.sentType === 'image');
      if (!hasImgSent) r.L += 1;
    }
  }
  ...
  // Lane decision
  for (const r of records.values()) {
    const isBroken = r.C > 0 || r.E > 0 || r.G >= 3 || r.grading > 0 || (r.yearShort && r.yearShort >= 5);
    const isSuspect = !isBroken && (r.G > 0 || r.F > 0 || r.L > 0 || (r.yearShort && r.yearShort > 0));
...
function main() {
  ...
  console.log(`  broken: ${byLane.broken}   suspect: ${byLane.suspect}   clean: ${byLane.clean}`);
  console.log(`  FREE:   broken ${byLaneFree.broken}, suspect ${byLaneFree.suspect}, clean ${byLaneFree.clean}`);
  console.log(`  LEGACY: broken ${byLaneLegacy.broken}, suspect ${byLaneLegacy.suspect}, clean ${byLaneLegacy.clean}`);
  console.log('');

  console.log('--- BROKEN FREE sets (release blocker) ---');
  for (const r of records.filter(x => x.lane === 'broken' && x.free)) {
    const tag = [r.C && `C`, r.E && `E`, r.G >= 3 && `G${r.G}`, r.grading && `grading${r.grading}`].filter(Boolean).join(',');
  ...
  for (const r of brokenLegacy.slice(0, 40)) {
    const tag = [r.C && `C`, r.E && `E`, r.G >= 3 && `G${r.G}`, r.grading && `grading${r.grading}`, r.yearShort && `year-${r.yearShort}`].filter(Boolean).join(',');
    console.log(`  ${r.yk}/${r.setId} (${r.range}) [${tag}]`);
  }
  if (brokenLegacy.length > 40) console.log(`  ... +${brokenLegacy.length - 40} more`);
  console.log('');

  console.log('--- SUSPECT FREE sets ---');
  for (const r of records.filter(x => x.lane === 'suspect' && x.free)) {
    const tag = [r.G && `G${r.G}`, r.F && `F${r.F}`, r.L && `L`].filter(Boolean).join(',');
    console.log(`  ${r.yk}/${r.setId} (${r.range}) [${tag}]`);
  }
  console.log('');
