...
const warn = (a, code, ctx = {}) =>
  a.push({ severity: "warning", code, ...ctx });
const nh = (a, code, ctx = {}) =>
  a.push({ severity: "needs_human", code, ...ctx });
...
function validateSents(set, warnings, errors) {
  ...
  if (!Array.isArray(set.sents))
    return { actualIds, canonicalIds, inferredBase: null };
  ...
  for (const sent of set.sents) {
    ...
    if (sent.sentType && !VALID_SENT_TYPES.has(sent.sentType)) {
      warn(warnings, "SENT_TYPE_UNKNOWN", {
        sentId: sent.id,
        value: sent.sentType,
      });
    }
...
function validateChoices(
  ...
  for (const c of q.choices) {
    ...
    if (typeof c.ok !== "boolean") {
      err(errors, "CHOICE_OK_NOT_BOOLEAN", {
        qid: q.id,
        num: c.num,
        value: c.ok,
      });
      ...
      for (const ref of c.cs_ids) {
        if (!sentSets.actualIds.has(ref)) {
          ...
          if (sentSets.canonicalIds.has(canonRef)) {
            ...
            err(errors, "CHOICE_CS_IDS_BROKEN_REF", {
              qid: q.id,
              num: c.num,
              ref,
            });
  ...
  if (q.questionType === "negative" && okFalse !== 1) {
    err(errors, "OK_DIST_NEGATIVE_EXPECT_1_FALSE", {
      qid: q.id,
      okTrue,
      okFalse,
    });
  } else if (q.questionType === "positive" && okTrue !== 1) {
    err(errors, "OK_DIST_POSITIVE_EXPECT_1_TRUE", {
      qid: q.id,
      okTrue,
      okFalse,
    });
  ...
  for (const v of set.vocab) {
    ...
    if (v.sentId && !sentSets.actualIds.has(v.sentId)) {
      ...
      if (sentSets.canonicalIds.has(canon)) {
        ...
        warn(warnings, "VOCAB_BROKEN_SENT_REF", {
          word: v.word,
          ref: v.sentId,
        });
      }
...
function validateSet(set, opts = {}) {
  ...
  const stats = {
    ...
    choices: (set.questions || []).reduce(
      (n, q) => n + (Array.isArray(q.choices) ? q.choices.length : 0),
      0,
    ),
    cs_ids_filled: (set.questions || []).reduce(
      (n, q) =>
        ...
        (Array.isArray(q.choices)
          ? q.choices.filter(
              (c) => Array.isArray(c.cs_ids) && c.cs_ids.length > 0,
            ).length
          : 0),
      0,
    ),
