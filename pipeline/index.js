...
const lastQuestionArg =
  arg5 && !SECTIONS.includes(arg5) ? parseInt(arg5) : null;
const section = SECTIONS.includes(arg5)
  ? arg5
  : SECTIONS.includes(arg6)
    ? arg6
    : "all";

if (!examPdfPath || !answerPdfPath || !examKey) {
  console.error(
    "사용법: node pipeline/index.js <시험지PDF> <정답PDF> <시험키> [섹션|마지막문항] [섹션]",
  );
  console.error(
    '예시1: node pipeline/index.js 시험지.pdf 정답.pdf "2023_6월" literature',
  );
  console.error(
    '예시2: node pipeline/index.js 시험지.pdf 정답.pdf "2023_6월" 34 literature',
  );
  console.error(
    '예시3: node pipeline/index.js 시험지.pdf 정답.pdf "2023수능"              ← 전체, 자동감지',
  );
...
if (!["reading", "literature", "all"].includes(section)) {
  console.error(
    `섹션은 "reading", "literature", "all" 중 하나여야 합니다. (받은 값: "${section}")`,
  );
  process.exit(1);
}
...
const skippedSections = ["reading", "literature"].filter(
  (s) => !targetSections.includes(s),
);
...
function loadCheckpoint() {
  ...
  if (fs.existsSync(p)) {
    ...
    console.log(
      `  📍 체크포인트 로드: step${cp.lastCompletedStep} 완료 상태 (${cp.timestamp})`,
    );
    return cp;
  }
...
function mergeSection(step5Data, sec, allDataPath) {
  ...
  const existingChoiceCount = existingSets.flatMap(
    (s) => s.questions?.flatMap((q) => q.choices) || [],
  ).length;
  const newChoiceCount = newSets.flatMap(
    (s) => s.questions?.flatMap((q) => q.choices) || [],
  ).length;
  if (
    existingChoiceCount > 0 &&
    Math.abs(newChoiceCount - existingChoiceCount) > 5
  ) {
    console.warn(
      `⚠️  선지 수 변동 큼: 기존 ${existingChoiceCount}개 → 새 ${newChoiceCount}개`,
    );
    console.warn(`   강제 진행하려면 --force 플래그를 사용하세요.`);
    if (!process.argv.includes("--force")) {
      throw new Error(
        `선지 수 변동이 너무 큼 (${existingChoiceCount} → ${newChoiceCount}). --force로 강제 실행 가능.`,
      );
    }
  }
  ...
  console.log(
    `     선지 ${totalC}개 | analysis ${filledA}/${totalC} | cs_ids ${filledC}/${totalC}`,
  );
}
      ...
      for (const q of set.questions ?? []) {
        for (const c of q.choices ?? []) {
          for (const csId of c.cs_ids || []) {
            if (!sentIds.has(csId)) {
              issues.push(
                `[${set.id}] Q${q.id}-${c.num}: DEAD cs_id → ${csId}`,
              );
            }
          }
        }
...
async function main() {
  ...
  // ★ lastQuestion 자동 감지
  const lastQuestion =
    lastQuestionArg ?? Math.max(...Object.keys(answerKey).map(Number));
  console.log(
    `  마지막 문항: ${lastQuestion}번 ${lastQuestionArg ? "(수동 지정)" : "(answer_key 자동 감지)"}`,
  );
  console.log(
    `  정답 ${Object.keys(answerKey).length}문항: ${JSON.stringify(answerKey)}`,
  );
  ...
  if (skipToStep <= 2) {
    ...
    step2Data = await extractStructure(
      examPdfPath,
      examKey,
      lastQuestion,
      section,
    ...
    for (const sec of targetSections) {
      ...
      if (fs.existsSync(cp))
        step2Data[sec] = JSON.parse(fs.readFileSync(cp, "utf8"));
    }
  }

  const step2Filtered = filterSections(step2Data);
  const totalSets = targetSections.reduce(
    (n, s) => n + (step2Filtered[s]?.length ?? 0),
    0,
  );
  ...
  if (skipToStep <= 5) {
    ...
    if (!step5Data) {
      const { result, stats } = await verifyAndFix(step4Data, answerKey, {
        step2Data: step2Filtered,
      });
      step5Data = result;
      ...
      if (stats.needsReview.length > 0) {
        console.warn(
          `  ⚠️  needsReview: ${stats.needsReview.map((r) => `[${r.setId}] ${r.qId}번`).join(", ")}`,
        );
      }
    }
    saveCheckpoint(5);
