...
def scan(set_id):
    ...
    for name, pno, x, a, b2, side in brs:
        ...
        #   거리 상한은 방향에 따라 다르다.
        #     side>0 (본문이 브래킷 오른쪽): 본문 x0 가 브래킷 바로 옆이다 — 실측 11~23pt.
        #       여기서 360 을 쓰면 **반대쪽 단까지 먹는다**(l20219a [A] 가 우단 문장을 먹어
        #       s1~s2 를 s13~s21 로 오판했다). 80 으로 좁힌다.
        #     side<0 (본문이 브래킷 왼쪽): 본문 x0 는 단 폭만큼 멀다 — l20276d 는 261.8pt.
        #       방향 조건이 반대쪽 단을 이미 걸러 주므로 360 을 유지한다.
        lim = 80 if side > 0 else 360
        cand = [it for it in cand
                if (it[3] > x - 20 if side > 0 else it[3] < x + 20) and abs(it[3] - x) < lim]
        if not cand:
            continue
