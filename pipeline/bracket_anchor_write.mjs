...
const SPEC = [
  // ── D-130 ④ 웨이브 3 배치 A 구간 표시 3세트 — 심사관 승인 ──────────
  //   셋 다 annotations.json 에 항목이 아예 없었다(ann 0). 본문에는 [A] 라는 글자만
  //   단독 문장으로 떠 있거나(l20219d·l20229d) 아예 없었다(r20216d).
  //   bracket_map_v2 로 지면 벡터를 읽고, 선지 내용과 교차 검증해 경계를 확정했다.
  ["2022_9월", "l20229d", "A", "l20229ds10", "l20229ds11",
    "11면 꺾쇠 983.6/1004.0 (x=738.3, 20.4pt = 2행). Q32#1 「여성의 생활에 밀접한 소재로 흘러가는 세월을 시각적으로」가 s10 「봄바람 가을 물이 베오리에 북 지나듯」(베짜기)과 s11 「설빈화안 어디 두고 면목가증 되거고나」에 정확히 대응한다"],
  ["2022_9월", "l20229d", "B", "l20229ds16", "l20229ds17",
    "12면 꺾쇠 180.4/200.8 (20.4pt = 2행). Q32#2 「단어를 반복하는 구절을 행마다 사용」이 결정적 근거다 — s16 「겨울밤 차고 찬 제」·s17 「여름날 길고 길 제」 두 행만 그 조건을 만족한다. 뒤 s18~s20 은 반복 구절이 없다"],
  ["2022_9월", "l20229d", "C", "l20229ds38", "l20229ds39",
    "12면 꺾쇠 584.7/605.1 (20.4pt = 2행). Q32#3 「두 대상을 발음이 비슷한 의태어로」가 s38 「소나무 … 흔덕흔덕」·s39 「버들 … 흔들흔들」 두 행에 정확히 대응한다"],
  ["2021_9월", "l20219d", "A", "l20219ds25", "l20219ds32",
    "12면 꺾쇠 162.7/604.4 (x=103.6). 라벨 글자가 s27 에 단독 문장으로 있어 도구 목록에서 빠졌으나 구간 안이다. Q32 선지들이 s25·s26(별전 입시·심봉사 진술)에서 s31·s32(황후가 말을 끊고 상봉)까지 걸쳐 있어 구간 전체를 포괄한다"],
  ["2021_6월", "r20216d", "A", "r20216ds15", "r20216ds20",
    "10면 꺾쇠 401.1/679.2 (x=453.1). 본문에 [A] 글자가 아예 없어 QG 가 「지문/bracket annotation 에 부재」로 잡던 건이다. s15~s20 은 지식 재산 보호의 최적 수준을 다루는 마지막 문단 전체이고, Q32 「[A]를 적용하여 <보기>를 이해」와 맞는다"],
  ...
  // ── D-115 ③ 웨이브 1 배치 1 ─────────────────────────────────────────
  ["2026_6월", "r20266b", "A", "r20266bs9", "r20266bs16",
    "2면 좌단 x=103.6 꺾쇠 493.4/734.3, 라벨 [A] y=609.2 가 구간 중앙(613.9). s9 y=489.2(+4.2) · s17 y=746.5 는 밖. 문항이 [A] 를 가리키는데 화면에 구간이 없던 세트(비노출)"],

  // ── D-118 ④ 배치 2 ────────────────────────────────────────────────
  ["2024_9월", "r20249a", "A", "r20249as13", "r20249as15",
    "1면 좌단 꺾쇠 714.3/806.6, 라벨 [A] y=755.9 가 구간 중앙(760.5). s13 y=710.0(+4.3) · s12 는 그 위 · s15 가 지문 마지막 문장이다. 문항이 [A] 를 가리키는데 ann·인라인·workTag 어디에도 없던 세트(비노출)"],
];
