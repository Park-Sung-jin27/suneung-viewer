...
const SPEC = [
  ...
  // ── D-155 ② 배치 B — 심사관 원본 실측 확정 ──────────────────────────
  //   본문에 [B] 가 workTag 문장(s55)으로 이미 떠 있으나 annotations 에 정박이 없어
  //   화면에 꺾쇠가 안 그려지던 건이다. 라벨 문장 바로 다음부터가 구간이다.
  ["2020_6월", "l20206d", "B", "l20206ds56", "l20206ds58",
    "심사관 원본 실측 확정 — 「하다가 별안간 방에서 뛰쳐나간다. / 맨발로 연못을 향해 몸을 날린다. / 그는 죽을 생각을 했던 것이다.」 3문장이 데이터 s56~s58 과 문장 경계까지 일치한다. 라벨 workTag 는 s55 로 구간 바로 앞에 있다. Q16#4·#5 가 s56·s57 을 근거로 쓰고 있어 선지 논리와도 맞는다"],

  // ── D-159 ② r2019b [A] — 심사관 원본 실측 확정 (451~457행 들여쓰기 블록) ──
  ["2019수능", "r2019b", "A", "r2019bs10", "r2019bs10",
    "원본 451~457행 들여쓰기 블록 「독자는, 그 수다스러운 점룡이 어머니가 … 경기 강화였다.」가 "
    + "데이터에서 s10 **한 문장**에 그대로 담겨 있다(시작·끝 문구 일치 확인). "
    + "s11 「이 봄에 대학 의과를 마친…」은 원본 464행으로 페이지가 넘어가 구간 밖이다. "
    + "Q23 이 [A]를 직접 묻는데 annotations 가 비어 있어 화면에 꺾쇠가 없던 세트"],
];
