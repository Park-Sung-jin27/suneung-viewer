    ...
    for (const sec of ['reading', 'literature']) {
      ...
      for (const s of yd[sec]) {
        for (const sent of (s.sents || [])) {
          ...
          if (noUnder !== sent.id) idx.set(noUnder, { t, yk, setId: s.id, sec });
          // also store with underscore alias
          const withUnder = sent.id.replace(/([a-z0-9]+)s(\d+)$/, '$1_s$2');
          if (withUnder !== sent.id) idx.set(withUnder, { t, yk, setId: s.id, sec });
        }
      }
    }
  }
      ...
      for (const e of arr) {
        ...
        if (t === 'bracket') {
          ...
          if (e.sentFrom && !sentIdx.has(e.sentFrom)) {
            findings.push({ yk, setId, issue: 'H_BRACKET_FROM_DEAD', sentFrom: e.sentFrom, label: e.label });
            sum.H += 1;
          }
          if (e.sentTo && !sentIdx.has(e.sentTo)) {
            findings.push({ yk, setId, issue: 'H_BRACKET_TO_DEAD', sentTo: e.sentTo, label: e.label });
            sum.H += 1;
          }
        } else {
          // F: sentId dead
          if (!e.sentId || !sentIdx.has(e.sentId)) {
            findings.push({ yk, setId, issue: 'F_SENT_DEAD', type: t, sentId: e.sentId, text: e.text, marker: e.marker });
            sum.F += 1;
            continue;
          }
          ...
          if (e.text && !sentInfo.t.includes(e.text)) {
            findings.push({ yk, setId, issue: 'G_TEXT_MISSING', type: t, sentId: e.sentId, text: e.text, sent_t_preview: sentInfo.t.slice(0, 80), marker: e.marker });
            sum.G += 1;
          }
        }
      }
...
function main() {
  ...
  fs.writeFileSync(outFile, JSON.stringify({ sum, findings, setsWithIssues, totalAnnoSets }, null, 2), 'utf-8');
  ...
  console.log(`F dead sentId: ${sum.F}  G text-missing: ${sum.G}  H bracket-dead: ${sum.H}  I set-empty: ${sum.I}  J set-not-in-data: ${sum.J}`);
  console.log('');
  ...
  for (const f of gList.slice(0, 10)) {
    ...
    console.log(`  ${f.yk}/${f.setId} ${f.type}${mk} sent=${f.sentId} text="${f.text}"`);
    console.log(`    sent_t: "${f.sent_t_preview}..."`);
  }
  console.log('');
  console.log('--- F samples (dead sentId) — first 10 ---');
  for (const f of findings.filter(f => f.issue === 'F_SENT_DEAD').slice(0, 10)) {
    console.log(`  ${f.yk}/${f.setId} ${f.type} sentId=${f.sentId} text="${f.text || ''}"`);
  }
  console.log('');
