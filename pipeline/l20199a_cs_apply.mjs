...
console.log("");
console.log("- 🔴 **데이터로는 검산이 안 된다.** (나) 구간에 연 구분 정보가 하나도 없다 —");
console.log("  `para` 를 쓰는 문학 세트는 98개 중 3개뿐이라 관례 자체가 없다.");
console.log("- 쉼표로 갈라 보려 했으나 **규칙이 성립하지 않는다**. 연 끝이 아닌 행도 쉼표로 끝난다:");
for (const sfx of ["s30", "s34", "s37", "s41", "s43"]) {
  const x = byId.get(SID + sfx);
  console.log(`  - ${sfx} \`${x.t}\`${sfx === "s41" ? "  ← **쉼표인데 연 끝이 아니다(반례)**" : ""}`);
}
console.log("- 판정하지 않는다(S-01). 지정된 세 문장은 (나) 본문 verse 가 맞고 형광펜도 켜지므로 그대로 적용하되,");
console.log("  **「1~3연 각 끝 행」인지는 원본 대조 전까지 미검증**으로 남긴다.");
console.log("");
