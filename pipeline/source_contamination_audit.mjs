...
//
// ★ 각주는 전문 일치로 본다 — 최소 길이 5자 (D-158 ②)
//   처음에 10자로 잡았더니 `*임원 : 산림.`(공백 제외 7자)을 놓쳤다.
//   그 각주는 r2021a 오염의 절반이었는데 s36 을 따라가 눈으로 찾아야 했다.
//   **각주는 원래 짧다** — 길이로 거르면 진짜를 놓친다. 5자로 낮추고 오탐은 실물로 확인한다.
//
// 진단만 한다. 아무것도 쓰지 않는다.
//
// 사용:
...
const NOTE_MIN = 5;   // 각주 최소 길이(공백 제외) — D-158 ② 로 10 → 5 하향
