...
for (const key of targets) {
  ...
  //   그래서 **마지막으로 등장하는 판정 기호(✅/❌)가 있는 줄**을 결론줄로 본다.
  {
    ...
    //
    //   ★ 옛 기준을 **전 데이터**에 돌리면 얼마나 틀리나 (D-164 ① 확정 근거)
    //     1,168건 / 212세트 — 그중 **LIVE 145세트**를 결함으로 부른다.
    //     세트 단위로만 돌릴 때는 「비표준 몇 건」으로 작아 보였다(배치 B 13세트에서 10건).
    //     축을 만들면 **반드시 전 데이터에 한 번 돌려 적발 규모를 본다**(S-25).
    //
    //   대신 **결론줄 끝 [라벨] ↔ pat 일치**를 본다 — D-164 ① 로 확정된 기준이다.
    //   근거는 실측 준수율이다: 라벨이 붙은 **612건 중 592건(96.7%)이 pat 과 일치**한다.
    //   준수율이 이만큼 높으니 어긋나는 20건(8세트 · LIVE 5)은 진짜 결함으로 볼 수 있다.
    //   S-13 표본 확인 3건 전건 실재 — 예: r20146c Q25#1 은 **어휘 문항**인데 결론줄이
    //   「부적절한 **어휘** 선택 [사실 왜곡]」로 자기모순이다(pat=V 가 맞고 라벨이 틀렸다).
    //   라벨이 없는 결론줄(3,070건)은 **판정하지 않는다** — 없는 것은 결함이 아니다.
