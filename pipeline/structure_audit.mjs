...
function audit() {
  ...
  const findings = [];
  const sum = { yearKeys: 0, sets: 0, questions: 0, A: 0, B: 0, C: 0, D: 0, E: 0 };
  ...
  for (const [yk, yd] of Object.entries(j)) {
    ...
    for (const sec of ['reading', 'literature']) {
      ...
      for (const s of sets) {
        ...
        if (idBad.length > 0) {
          findings.push({ yk, sid, sec, issue: 'A', count: idBad.length, total: qs.length });
          sum.A += 1;
        }
        if (qs.length === 0) {
        ...
        if (s.range && qs.length > 0) {
          ...
          if (m) {
            ...
            const act = qs.map(q => q.id).filter(x => Number.isInteger(x)).sort((a, b) => a - b);
            const same = act.length === exp.length && act.every((v, i) => v === exp[i]);
            if (!same) {
              findings.push({ yk, sid, sec, issue: 'E', range: s.range, expected: exp, actual: act });
              sum.E += 1;
            }
          }
        }
...
function main() {
  ...
  fs.writeFileSync(outFile, JSON.stringify({ sum, findings }, null, 2), 'utf-8');

  console.log('=== STRUCTURE AUDIT', today, '===');
  console.log('yearKeys:', sum.yearKeys, ' sets:', sum.sets, ' questions:', sum.questions);
  console.log('A q.id-bad:', sum.A, ' B q=0:', sum.B, ' C sents=0:', sum.C, ' D year<25:', sum.D, ' E range-mismatch:', sum.E);
  console.log('');
  console.log('--- D (year < 25 questions) ---');
  for (const f of findings.filter(f => f.issue === 'D').sort((a, b) => a.total - b.total)) {
  ...
  for (const f of eList.slice(0, 30)) {
    console.log(`  ${f.yk}/${f.sid} range=${f.range} expected=[${f.expected.join(',')}] actual=[${f.actual.join(',')}]`);
  }
  if (eList.length > 30) console.log(`  ... +${eList.length - 30} more`);
  console.log('');
  console.log('--- C (sents=0) ---');
  for (const f of findings.filter(f => f.issue === 'C')) console.log(`  ${f.yk}/${f.sid} range=${f.range}`);
  console.log('');
