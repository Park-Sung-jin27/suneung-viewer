...
const Q_ID =
  ARG3 && ARG3 !== "--all" && !Number.isNaN(parseInt(ARG3, 10))
    ? parseInt(ARG3, 10)
    : null;
const C_NUM =
  ARG4 && !Number.isNaN(parseInt(ARG4, 10)) ? parseInt(ARG4, 10) : null;
const ALL_MODE = ARG3 === "--all";

if (!YEAR || !SET_ID) {
  console.error(
    "사용법: node pipeline/gen_qa_payload.js <연도> <세트ID> [문제번호] [선지번호]",
  );
...
function normalizeBogi(bogi) {
  ...
  try {
    return JSON.stringify(bogi, null, 2);
  } catch {
    return String(bogi);
  }
}
...
function getSentTexts(choice) {
  ...
  const lines = cs_ids.map(
    (id, i) => `${i + 1}. [${id}] ${sentMap[id] || "(문장 없음)"}`,
  );
  if (Array.isArray(choice.cs_spans) && choice.cs_spans.length > 0) {
    ...
    choice.cs_spans.forEach((sp, i) => {
      lines.push(
        `${i + 1}. sent_id=${sp.sent_id}, text=${sp.text || ""}${sp.occurrence ? `, occurrence=${sp.occurrence}` : ""}`,
      );
    });
  }
  return lines.join("\n");
...
console.log(`\n${"═".repeat(70)}`);
console.log(
  `${YEAR} ${SET_ID} ${ALL_MODE ? "전체" : `Q${Q_ID}${C_NUM ? ` 선지${C_NUM}` : ""}`} — 통합 QA Payload`,
);
