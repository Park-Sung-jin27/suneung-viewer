...
RULE_1 엄격 적용 lock (필수 — P_MISMATCH 과민 사양 방지):
- RULE_1 발동 사양 = **choice 자체** 안 pat 정의 위배 사양 단독
- 발동 사양 X 사실 (RULE_1 hit 금지) = choice가 pat 정의 정합 사양 안, analysis 안 근거 약함 / 조건 누락 / 논리 vague 사양 단독 사양
- analysis 안 근거 사양 결함 = RULE_2 (직접성) / RULE_3 (조건 누락) / RULE_7 (논리 vague) 영역 사양

판정 사양 (실예):
- 사양 (RULE_1 발동 X): pat=R1 + choice 안 사실 왜곡 사실 (네트워크 효과 부정 등) + analysis 안 근거 약함 → RULE_2/3 단독 hit, RULE_1 hit X
- 사양 (RULE_1 발동): pat=R1 + choice 안 인과 전도 사실 (R2 구조) → RULE_1 + RULE_4 hit + P_MISMATCH

메타-고백 감지 lock (필수 — pat 정합 판정 우선):
analysis 본문 안 다음 자기 고백 표현이 직접 인용된 경우, pat이 정합으로 보이더라도 사양 우위 판정 의무:

(a) 선지 조건 누락 자기 고백 → RULE_3_CONDITION_MISSING + error_type=E_CONDITION_MISSING
  - "직접 검토하지 않는다" / "별도로 다루지 않는다" / "본 판정에서 제외한다"
  - "두 번째 조건은 ~" / "나머지 조건은 ~" 등 부분 처리 명시
  - 사양: 선지가 복합 조건 (A and B, A or B 등) 안 일부 단독 분석한 사양

(b) 근거→판단 논리 누락 자기 고백 → RULE_7_ANALYSIS_TOO_VAGUE + error_type=E_LOGIC_UNCLEAR
  - "어떻게 충돌하는지 설명하지 않는다"
  - "왜 어긋나는지 명시하지 않는다"
  - "근거와 선지의 연결은 ~ 별도"
  - 사양: 근거는 정확 인용하나, 근거에서 선지 판단으로 이어지는 논리 단계 누락 사양

판정 사양:
- 위 (a)/(b) 사양 사실 시 = analysis 안 자기 고백이 본문에 직접 인용된 경우에만 발동
- 추정·해석 사양 안 발동 금지 (예: 단순히 짧다고 RULE_7 발동 금지)
- 본 lock 사양 안 발동 시 = pat이 정합으로 보이더라도 error_type 우선 적용 (R1/R2/etc. 단순 정합 사양 안 우선)
