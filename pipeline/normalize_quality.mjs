...
function detectPat(analysis, sec) {
  ...
  if (
    /팩트 왜곡|사실 왜곡|의미 왜곡|어휘 의미|문맥적 의미|어휘.*범위|어휘.*오답|정반대|역전된/.test(
      a,
    )
  )
  ...
  if (
    /관계[··]인과|인과 전도|인과관계 왜곡|논리 왜곡|논리.*역전|반박.*지지|반박-지지|대상 바꿔치기|순서 역전|단계.*오대응|반대 상황/.test(
      a,
    )
  )
  ...
  if (
    /과도한 추론|과잉 추론|지문에 없|근거 부재|지문 핵심 미파악|과장 해석|용어 불일치|불완전.*대응/.test(
      a,
    )
  )
  ...
  if (
    /수사법|시어|이미지|표현법|시간 표지|표현 방식/.test(a) &&
    sec === "literature"
  )
    return "L1";
  ...
  if (
    /구조.*오류|맥락.*오류|장면 전환|서술 방식|화자.*오독|주체 오독|인물 및 행위|인물.*오인|공간.*오독/.test(
      a,
    ) &&
    sec === "literature"
  )
  ...
  if (
    /정서.*오류|태도 오독|화자의 태도 오독|감정.*오독/.test(a) &&
    sec === "literature"
  )
    return "L2";
  ...
  // 주제·의미 과잉 → L3
  if (
    /권면 대상|핵심 의미 왜곡|과도한 의미|주제.*확대/.test(a) &&
    sec === "literature"
  )
    return "L3";
          ...
          if (typeof c.pat === "number") {
            if (c.ok === true) {
              ...
              if (detected) {
                ...
                console.warn(
                  `⚠️  분류불가 → null: ${yk} ${set.id} Q${q.id}C${c.num} | ${ana.slice(-80)}`,
                );
              }
            }
