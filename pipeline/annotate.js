...
if (!yd) {
  console.error(`❌ 연도키 "${yearKey}" 없음. 존재하는 키: ${Object.keys(allData).join(', ')}`);
  process.exit(1);
}
...
for (const sec of ['reading', 'literature']) {
  for (const set of (yd[sec] || [])) {
    ...
    for (const sent of (set.sents || [])) {
      const preview = (sent.text || sent.t || '').slice(0, 60).replace(/\n/g, ' ');
  ...
  for (const [setId, items] of Object.entries(entry)) {
    ...
    for (const item of items) {
      if (item.type === 'bracket') {
        console.log(`    bracket  [${item.label}]  ${item.sentFrom} ~ ${item.sentTo}`);
      } else {
        console.log(`    ${item.type.padEnd(10)}  ${item.sentId.padEnd(22)}  "${item.text}"`);
      }
    }
  }
} else {
...
console.log(`       "${yearKey}": {`);
console.log(`         "s1": [`);
console.log(`           { "type": "box",       "sentId": "r2026a_s3",   "text": "핵심 개념" },`);
console.log(`           { "type": "underline",  "sentId": "r2026a_s7",   "text": "어휘" }`);
console.log(`         ],`);
console.log(`         "l2026a": [`);
console.log(`           { "type": "bracket", "label": "A", "sentFrom": "l2026as4", "sentTo": "l2026as8" }`);
console.log(`         ]`);
console.log(`       }`);
console.log(`     }`);
