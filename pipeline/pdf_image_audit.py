...
def scan_pdf(path):
    ...
    return len(keep), total, {"repeated": len(repeated), "small": len(small)}, sample
...
def main():
    ...
    md.append("")
    md.append("| 회차 | 유의미 | 전체 | 반복제외 | 소형제외 | 데이터 선언 | 큰 이미지 표본 (w×h @쪽) |")
    md.append("|---|--:|--:|--:|--:|--:|---|")
