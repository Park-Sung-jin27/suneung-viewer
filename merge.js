...
try {
  ...
  const newSets = Array.isArray(updatedData) ? updatedData : Object.values(updatedData);
  ...
  newSets.forEach(newSet => {
    ...
    if (idx !== -1) {
      ...
      console.log(`[성공] ${newSet.title} (${newSet.range}) 문학 데이터 업데이트 완료!`);
    } else {
      console.log(`[알림] 기존에 없던 ${newSet.title} 데이터를 새로 추가합니다.`);
