# Code A 작업 지시 — /privacy route 추가 (개인정보처리방침 + 이용약관)

> 작성: 품질 심사관 (2026-05-15) · 사용자 검토 후 paste
> 목표: 베타 발송 trigger 1번 조건 충족 — `/privacy` route 신규
> 본문은 사용자 = 성진 제공 정보 (사업자 297-93-01982 외) 정합 반영
...
깨끗 상태 확인 후 진행. 사전 라이브 status: commit `32faaac` (KPI 1차 패치) 이후 추가 commit 없음.

---

## Step 1. 신규 파일 — `src/Privacy.jsx`
  ...
  return (
    ...
    >
      ...
      <section style={SECTION_STYLE}>
        ...
        <p style={P_STYLE}>
          짚이(이하 "서비스")는 「개인정보 보호법」을 정합하여 이용자의
...
## Step 2. 편집 — `src/App.jsx`

**Action**: Edit (2 위치)

### 2-1. import 추가

기존 (다른 component import 부근, 예: `import FeedbackButton from "./FeedbackButton";` 근처):
...
변경 (`/privacy` route 추가 — wildcard 직전에 위치 의무):
...
**위치 lock 의무**: `path="*"` wildcard 보다 **위** 에 위치해야 함. 아래 두면 wildcard 가 catch all 해서 홈으로 redirect 됨 (현 버그 원인).

---

## Step 3. 편집 — `src/Landing.jsx` 또는 footer 영역 (선택)

**Action**: Edit (선택 — landing footer 에 /privacy 링크 노출)

> 본 step 은 선택. /privacy route 만 추가하면 직접 URL 진입 가능. 단, 베타 모집 form 신뢰성 향상 위해 landing 하단에 링크 노출 권고.

### 3-1. Landing.jsx footer 영역 찾기

```bash
grep -n "footer\|©\|copyright\|회사" src/Landing.jsx
```
...
(footer 가 명시적 없으면 본 step skip, 사용자에게 별도 보고)
...
error 0 의무. error 시 lint·type error 보고.
...
브라우저 → `http://localhost:5173/privacy` 진입.

- ✓ 페이지 노출 (홈 redirect 안 됨)
- ✓ 사업자 정보 표 표시 (297-93-01982 등)
- ✓ "1. 개인정보처리방침" 본문 표시
- ✓ "2. 이용약관" 본문 표시
- ✓ Header 정상 노출 (Layout wrap 정합)
- ✓ 우측 하단 "💬 베타 피드백" floating button 노출

### 검증 3. 배포 라이브 (push 후)

배포 후 (Vercel auto-deploy 1~3분 대기):

- ✓ `https://suneung-viewer.vercel.app/privacy` 200 OK
- ✓ 직접 URL 진입 시 홈으로 redirect 안 됨
...
```
[완료 보고]
작업: /privacy route 추가 (Privacy.jsx + App.jsx 패치)
완료 기준 충족: Y/N
결과:
  - npm run build: error 0
  - 라이브 /privacy 200 OK
  - 사업자 정보 + 개인정보처리방침 + 이용약관 노출 ✓
문제:
다음 액션 1개:
```
...
- **변호사 검토 후순위 path** [Confirmed, CLAUDE.md 5/8 결정]. 베타 14일 case study 사후 결정
- **만 14세 미만 사용자 path** 베타 단계 약관 안 명시 only. 본격 운영 전 가입 form 안 차단 시스템 구축 의무 — 별도 회기 lock
- **결제 page 활성화 시점** 까지 통신판매업 신고 의무 영역 사실 확인 의무 — 사용자 영역
- **Toss Payments 정합** 결제 page 활성화 시 결제대행사 약관 정합 path 추가 의무
