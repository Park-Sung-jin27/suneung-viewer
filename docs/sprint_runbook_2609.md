...
| **8-3** 품질 게이트 | `MARKER_INTEGRITY_FAIL` · `CS_ALL_NONHIGHLIGHTABLE` · `BOGI_IMAGE_MISSING` · `F_content_reversed` · `OK_ANALYSIS_CONFLICT` → 🟠 **세트 제외**<br>그 밖의 CRITICAL → ✅ **진행 + 기록** | 앞 다섯은 화면이 깨지거나 정답을 거꾸로 설명한다 |
