...
3. **비노출 text 환각 worklist**: structure_fidelity sim<0.3 비노출군(2017_9월 l20179a/b/c 8건 등 자기완결 환각 의심). 표/학습지 문항은 pdftotext sim 항상 낮아 진짜/FP 구분 불가 → **렌더 이미지 직독 필수**(Phase 4 비노출).
4. **r2014e Q30 보기표** PDF 대조 + 해설(patch 대기, patch_r2014e_recon.json 지시 4·5).
5. **l2015bB (나) 유한라산기 본문 누락 복원**(비노출, _done/2015수능B PDF 전사).
