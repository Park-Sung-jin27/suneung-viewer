...
1. **(a) 자동 대조 diff 전건 판정** — `passage_fidelity` 자동 대조가 낸 diff 를 한 건도 남기지 않고
   사람이 판정한다. 각 diff 를 **진짜 오타** 또는 **추출 artifact**(古語·한양PUA·줄바꿈·공백)로 분류하고
   판정 근거를 남긴다. **미판정 diff 가 1건이라도 있으면 통과가 아니다.**
2. **(b) 마커·각주·보기·문단 구분의 위치 대조** — 마커(㉠~㉤ · ⓐ~ⓔ)·각주·`<보기>`·문단 구분이
