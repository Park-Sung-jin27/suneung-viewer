...
function auditSet(set, annList) {
  ...
  for (const ms of bodyMarkerMap.values())
    ms.forEach((m) => bodyMarkers.add(m));
  ...
  for (const m of stemSet) {
    if (!bodyMarkers.has(m)) {
      const qs = qMarkers
        .filter((q) => q.stemMarkers.includes(m))
        .map((q) => q.qid);
  ...
  for (const m of choiceSet) {
    if (!bodyMarkers.has(m) && !bogiSet.has(m)) {
      const qs = qMarkers
        .filter((q) => q.choiceMarkers.includes(m))
        .map((q) => q.qid);
      issues.marker_body.push({
  ...
  for (const m of bogiSet) {
    const qs = qMarkers
      .filter((q) => q.bogiMarkers.includes(m))
      .map((q) => q.qid);
  ...
  const passageUnderlines = underlines.filter(
    (u) => !u.target || u.target === "passage",
  );
  ...
  if (bodyMarkerCount > 0 && passageUnderlines.length < bodyMarkerCount) {
    issues.underline.push({
      kind: "count_mismatch",
      detail: `본문 marker ${bodyMarkerCount}개 vs passage underline ${passageUnderlines.length}개 — ${bodyMarkerCount - passageUnderlines.length}개 누락 (info)`,
    });
  }
...
export default function AuditPanel({ user }) {
  ...
  if (yearKeyParam) {
    ...
    if (!chosen) {
      return (
        <div style={{ padding: 20 }}>
          ...
          <Link to={`/audit/${setId}`}>yearKey 선택 화면으로</Link> ·{" "}
          <Link to="/">홈으로</Link>
        </div>
      );
    }
    ...
    return (
      <div style={{ padding: 20, fontFamily: "system-ui, sans-serif" }}>
        ...
        <p>
          <code>{setId}</code> 는 {matches.length}개 yearKey 에 존재합니다.
          검수할 yearKey 를 선택하세요.
        </p>
        <ul>
          {matches.map((m) => (
            <li key={m.yearKey} style={{ margin: "8px 0" }}>
              <Link
                to={`/audit/${setId}?yearKey=${encodeURIComponent(m.yearKey)}`}
              >
                <strong>{m.yearKey}</strong> ({m.area}) — sent{" "}
                {m.set.sents?.length || 0}, Q {(m.set.questions || []).length}
              </Link>
            </li>
          ))}
        </ul>
  ...
  const clearStaged = () => {
    setStagedItems([]);
    setBracketStartSent(null);
    setBracketEndSent(null);
  };

  const handleAddUnderline = () => {
    if (!activeSentId || !uText.trim()) {
      alert("sent 를 좌측에서 클릭하고 text 를 입력하세요.");
      return;
    }
    addStaged({
      type: "underline",
  ...
  const handleAddMarker = () => {
    ...
    addStaged({
      ...
      marker: mkMarker || "",
      text: mkText.trim(),
      endMarker: mkEndMarker,
    });
    setMkMarker("");
    setMkText("");
    setMkEndMarker(false);
  };

  const handleAddBracket = () => {
    if (!bracketStartSent || !bracketEndSent || !brLabel.trim()) {
      alert("시작 sent / 끝 sent / label 을 모두 입력하세요.");
      return;
    }
    addStaged({
      type: "bracket",
      label: brLabel.trim(),
      sentFrom: bracketStartSent,
      sentTo: bracketEndSent,
    });
    setBracketStartSent(null);
    setBracketEndSent(null);
    setBrLabel("");
  ...
  const handleAddBox = () => {
    if (!boxMarker && !boxLabel) {
      alert("marker 또는 label 중 하나는 입력하세요.");
      return;
    }
  ...
  const removeDeletion = (idx) =>
    setStagedDeletions((prev) => prev.filter((_, i) => i !== idx));
  ...
  const buildJsonSpec = () => {
    ...
    return JSON.stringify(
      { setId, year: foundYear, additions: stagedItems, deletions },
      null,
      2,
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1
          style={{ fontSize: 22, fontWeight: 800, color: "#1f2937", margin: 0 }}
        >
          🔍 Audit: {setId}
        </h1>
      ...
      >
        <div>
          <Section title="본문 sents (클릭 → 활성 sent 선택 · marker 강조)">
            ...
            >
              {foundSet.sents.map((sent) => {
                ...
                return (
                  ...
                  >
                    <code
                      style={{ color: "#6b7280", fontSize: 11, marginRight: 8 }}
                    >
                      {sent.id}
                    </code>
            ...
            {activeSentId && (
              ...
              >
                <span>
                  활성 sent: <code>{activeSentId}</code>
                </span>
                <button
                  type="button"
                  onClick={() => setBracketStartSent(activeSentId)}
                ...
                <button
                  type="button"
                  onClick={() => setBracketEndSent(activeSentId)}
                  style={btnSmStyle("#6366f1")}
                >
                  [Bracket End]
                </button>
                <button
                  type="button"
                  onClick={() => setActiveSentId(null)}
                  style={btnSmStyle("#9ca3af")}
                >
                  선택 해제
                </button>
              </div>
          ...
          <Section title="발문 / 선지 / bogi marker">
            ...
            >
              {foundSet.questions.map((q) => {
                ...
                const choiceMarkers = (q.choices || []).flatMap(
                  (c) =>
                ...
                const allM = [
                  ...new Set([...qMarkers, ...bogiMarkers, ...choiceMarkers]),
                ].sort();
        ...
        <div>
          ...
          <Section
            title={`📋 기존 annotation 목록 (${annList.length}건) — 삭제 staging`}
          >
            <div
              style={{
                background: "#fff",
            ...
            >
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: 6,
                        marginBottom: 4,
                        background: staged ? "#fee2e2" : typeColor(ann.type),
                        borderRadius: 4,
                        display: "flex",
                        justifyContent: "space-between",
                        alignItems: "flex-start",
                        gap: 6,
                        opacity: staged ? 0.6 : 1,
                      }}
                    >
                      <div style={{ flex: 1, wordBreak: "break-all" }}>
                        ...
                        {ann.sentId && (
                          <code style={{ color: "#6b7280" }}>{ann.sentId}</code>
                        )}
                        {ann.sentFrom && (
                          <code style={{ color: "#6b7280" }}>
                            {ann.sentFrom}~{ann.sentTo}
                          </code>
                        )}
                        {ann.target && (
              ...
              {stagedDeletions.length > 0 && (
                ...
                >
                  ...
                  >
                    삭제 staging: {stagedDeletions.length}건
                  </div>
                  <button
                    type="button"
                    onClick={clearDeletions}
                    style={btnSmStyle("#6b7280")}
                  >
                    모두 취소
                  </button>
                </div>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 12 }}>
          입력값은 실제 데이터에 적용되지 않습니다. staging → 클립보드 복사 →
          채팅 paste → AI 일괄 처리.
        </div>
        ...
        >
          <div>
            <FormBox title="① underline 추가">
              <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4 }}>
                활성 sent:{" "}
                {activeSentId ? (
                  <code style={{ color: "#1e40af" }}>{activeSentId}</code>
                ) : (
                  <span style={{ color: "#dc2626" }}>(좌측에서 sent 클릭)</span>
                )}
              </div>
              <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                <select
                  value={uMarker}
                  onChange={(e) => setUMarker(e.target.value)}
                ...
                <textarea
                  ...
                  placeholder="text (본문 그대로 verbatim)"
            ...
            <FormBox title="② marker 추가 (단순 marker 표시)">
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                <select
                  value={mkMarker}
                  onChange={(e) => setMkMarker(e.target.value)}
                  style={inputStyle({ width: 70 })}
                >
                  <option value="">(no marker)</option>
                  {MARKER_CHOICES.map((m) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={mkEndMarker}
                  onChange={(e) => setMkEndMarker(e.target.checked)}
                />{" "}
                endMarker
              </label>
              <button
            ...
            <FormBox title="③ bracket 추가">
              <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4 }}>
                시작 sent:{" "}
                {bracketStartSent ? (
                  <code style={{ color: "#4338ca" }}>{bracketStartSent}</code>
                ) : (
                  <span style={{ color: "#dc2626" }}>
                    (좌측 sent 클릭 → [Bracket Start])
                  </span>
                )}
                {" / "}끝 sent:{" "}
                {bracketEndSent ? (
                  <code style={{ color: "#4338ca" }}>{bracketEndSent}</code>
                ) : (
                  <span style={{ color: "#dc2626" }}>(→ [Bracket End])</span>
                )}
              </div>
              <input
                type="text"
                value={brLabel}
                onChange={(e) => setBrLabel(e.target.value)}
                placeholder="label (예: A, B, C)"
            ...
            <FormBox title="④ blank-box 추가">
              <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                <select
                  value={boxMarker}
                  onChange={(e) => setBoxMarker(e.target.value)}
                  style={inputStyle({ width: 70 })}
                >
                  <option value="">(no marker)</option>
                  {MARKER_CHOICES.map((m) => (
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                ...
                {boxTarget === "bogi" && (
                  <input
                    type="text"
                    value={boxQid}
                    onChange={(e) => setBoxQid(e.target.value)}
                    placeholder="qId"
                    style={inputStyle({ width: 70 })}
                  />
                )}
          ...
          <div>
            <FormBox
              title={`📦 Staging (${stagedItems.length}건)`}
              right={
            ...
            >
              {stagedItems.length === 0 ? (
                <div style={{ color: "#9ca3af", fontSize: 12 }}>
                  staging 비어있음 — 좌측 폼으로 추가
                </div>
              ) : (
                <div
                  style={{ maxHeight: 200, overflowY: "auto", fontSize: 12 }}
                >
                  {stagedItems.map((it, i) => (
                    ...
                    >
                      <div style={{ flex: 1, wordBreak: "break-all" }}>
                        ...
                        {it.sentId && (
                          <code style={{ color: "#6b7280" }}>{it.sentId}</code>
                        )}
                        {it.sentFrom && (
                          <code style={{ color: "#6b7280" }}>
                            {it.sentFrom} ~ {it.sentTo}
                          </code>
                        )}
                        {it.width && (
                          <span style={{ marginLeft: 6, color: "#374151" }}>
            ...
            <FormBox
              ...
              right={
                <button
                  type="button"
                  onClick={() => copyToClipboard(buildTextSpec())}
                  disabled={stagedItems.length === 0}
                  style={btnSmStyle(
                    stagedItems.length === 0 ? "#9ca3af" : "#2563eb",
                  )}
                >
                  클립보드 복사
                </button>
            ...
            <FormBox
              ...
              right={
                <button
                  type="button"
                  onClick={() => copyToClipboard(buildJsonSpec())}
                  disabled={stagedItems.length === 0}
                  style={btnSmStyle(
                    stagedItems.length === 0 ? "#9ca3af" : "#2563eb",
                  )}
...
function FormBox({ title, right, children }) {
  return (
    ...
    >
      ...
      >
        <h4
          style={{ fontSize: 12, fontWeight: 700, color: "#374151", margin: 0 }}
        >
          {title}
        </h4>
        {right || null}
          ...
          {issues.map((iss, i) => (
            <div
              ...
              style={{
                ...
                color: "#1f2937",
                borderBottom:
                  i < issues.length - 1 ? "1px dashed #f3f4f6" : "none",
              }}
            >
...
function CsIdsReviewSection({
  ...
  if (!csCandidates) {
    return (
      <Section title="🔗 cs_ids 후보 검토 (v3.1)">
        ...
        >
          cs_ids_candidates.json 로딩 안 됨. `pipeline/cs_ids_recovery.mjs` 실행
          + `public/audit_data/cs_ids_candidates.json` 복사 필요.
        </div>
      </Section>
    );
  }
  ...
  const byDecision = {
    ...
    no_quote_extractable: setCands.filter(
      (c) => c.decision === "no_quote_extractable",
    ),
    duplicate_sentid_hold: setCands.filter(
      (c) => c.decision === "duplicate_sentid_hold",
    ),
  };
  // v2: duplicate sentId hold 표시 — 자동/배치 금지
  const isDuplicateHold =
    byDecision.duplicate_sentid_hold.length > 0 ||
    setCands.some((c) => c.set_safety === "duplicate_sentid_hold");
  ...
  const approve = (c, sentId, score) => {
    setCsApprovals((prev) => ({
      ...
      [approvalKey(c)]: {
        sentId,
        score,
        quote: c.candidates?.[0]?.quotes_matched?.[0] || null,
      },
    }));
  };
  const unapprove = (c) => {
  ...
  return (
    <Section
      title={`🔗 cs_ids 후보 검토 (${setCands.length}건) — 승인 ${approvedCount}건`}
    >
      {isDuplicateHold && (
        <div
          style={{
        ...
        >
          ⚠️ duplicate_sent_id 결함: 본 set 안 sentId 중복 — cs_ids 자동/배치
          반영 금지. sentId 재매핑 sprint 후 처리.
        </div>
      )}
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            auto: {byDecision.auto_apply.length}
          </span>
          <span
            style={{
              background: "#fef3c7",
          ...
          >
            batch: {byDecision.batch_review.length}
          </span>
          <span
            style={{
              background: "#fee2e2",
          ...
          >
            manual: {byDecision.manual_needed.length}
          </span>
          <span
            style={{
              background: "#f3f4f6",
          ...
          >
            no_quote: {byDecision.no_quote_extractable.length}
          </span>
          {approvedCount > 0 && (
            <button
              type="button"
          ...
          return (
            ...
            >
                ...
                <div>
                  ...
                  <span
                    style={{
                      ...
                      fontSize: 11,
                      background:
                        c.decision === "batch_review" ? "#fef3c7" : "#fee2e2",
                      color:
                        c.decision === "batch_review" ? "#92400e" : "#991b1b",
                      padding: "1px 6px",
                      borderRadius: 3,
                      fontWeight: 700,
                    }}
              ...
              {c.analysis_quotes?.length > 0 && (
                <div
                  style={{ fontSize: 11, color: "#374151", marginBottom: 6 }}
                >
                  💬 인용문:{" "}
                  {c.analysis_quotes.map((q, i) => (
                    <span
                      key={i}
                      style={{
                        background: "#fef9c3",
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          alignItems: "flex-start",
                          padding: "4px 6px",
                          background: isApproved ? "#dcfce7" : "transparent",
                          borderRadius: 3,
                          marginBottom: 2,
                        }}
                      >
                        <input
                          ...
                          checked={isApproved}
                          onChange={() =>
                            approve(c, cand.sentId, cand.normalized_score)
                          }
                          style={{ marginTop: 4 }}
                        />
                        <div style={{ flex: 1, fontSize: 12 }}>
                          ...
                          <span
                            style={{
                              ...
                              fontSize: 10,
                              color:
                                cand.normalized_score >= 0.8
                                  ? "#16a34a"
                                  : cand.normalized_score >= 0.5
                                    ? "#ca8a04"
                                    : "#dc2626",
                              fontWeight: 700,
                            }}
        ...
        {byDecision.batch_review.length === 0 &&
          byDecision.manual_needed.length === 0 && (
            <div style={{ color: "#9ca3af", fontSize: 12 }}>
              이 set 은 batch/manual 후보 없음 (auto 만 또는 후보 없음)
            </div>
          )}
      </div>
    </Section>
