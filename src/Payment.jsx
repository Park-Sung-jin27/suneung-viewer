...
function PlanCard({ plan, selected, onSelect, tossPayments, loading, onPay, onFreeStart }) {
  const isFree     = plan.price === 0;
  const isFeatured = plan.id === 'standard';
  const [hov, setHov] = useState(false);

  function handleCta() {
    if (isFree) { onFreeStart?.(); return; }
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isFeatured ? `2px solid #1e5c1e` : `1px solid ${selected === plan.id ? C.green : C.border}`,
        borderRadius: '18px',
        padding: '26px 22px',
        position: 'relative',
        overflow: 'hidden',
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '22px' }}>
        {plan.features.map((f, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 9 }}>
            <span style={{
              color: f.ok
                ? (isFeatured ? 'rgba(255,255,255,0.7)' : C.line)
                : (isFeatured ? 'rgba(255,255,255,0.2)' : '#d1d5db'),
            ...
            <span style={{
              fontSize: '0.81rem',
              color: f.ok
                ? (isFeatured ? 'rgba(255,255,255,0.9)' : C.ink)
                : (isFeatured ? 'rgba(255,255,255,0.3)' : C.subtle),
            }}>
              {f.text}
            </span>
          </div>
      ...
      {isFree ? (
        <button
          onClick={e => { e.stopPropagation(); onFreeStart?.(); }}
          style={{
        ...
        <div style={{
          ...
          border: isFeatured ? '1px solid rgba(255,255,255,0.15)' : `1px solid ${C.border}`,
  ...
  async function handlePay(plan) {
    ...
    try {
      await tossPayments.requestPayment({
        ...
        successUrl: window.location.origin + '/?paymentKey={paymentKey}&orderId={orderId}&amount={amount}',
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '48px' }}>
        <div style={{
          ...
          color: C.green, background: C.bg, border: `1px solid ${C.line}35`,
      ...
      <div style={{
        ...
        {[
          { icon: '🛡️', title: '30일 전액 환불', desc: '이유 불문, 문의 한 통으로 완료' },
          { icon: '🔓', title: '신용카드 없이 시작', desc: '14일 무료 체험 — 결제 정보 불필요' },
          { icon: '📄', title: '데이터는 내 것', desc: '해지 후에도 리포트 PDF 다운로드 유지' },
        ].map((g, i) => (
          <div key={i} style={{
            ...
            <div>
              <div style={{ fontSize: '0.82rem', fontWeight: '700', color: C.ink, marginBottom: '4px' }}>{g.title}</div>
              <div style={{ fontSize: '0.75rem', color: C.muted, lineHeight: 1.6 }}>{g.desc}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* 안내 */}
      <p style={{
        fontSize: '0.72rem', color: C.subtle,
        textAlign: 'center', lineHeight: '1.7',
      }}>
        스탠다드·프리미엄 플랜은 결제 시스템 준비 완료 후 순차 오픈 예정입니다.<br />
