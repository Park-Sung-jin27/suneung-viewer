// ============================================================
// QuizPanel.jsx (업데이트 — BogiTable 통합)
// ============================================================
...
function PatternBadge({ pat }) {
  ...
  const p = P[pat];
  return (
    <span style={{ display:'inline-block', fontSize:'0.7rem', fontWeight:'700', color:p.color, background:p.bg, border:`1px solid ${p.color}40`, borderRadius:'4px', padding:'1px 7px', marginLeft:'6px', verticalAlign:'middle' }}>
      P{pat} {p.name}
    </span>
  );
}
...
function ChoiceItem({ choice, qid, sel, onSelect, revealed }) {
  ...
  if (isSelected) { bg=choice.ok?'#ecfdf5':'#fef2f2'; border=`1.5px solid ${choice.ok?'#10b981':'#ef4444'}`; textColor=choice.ok?'#065f46':'#7f1d1d'; }
  else if (showCorrect) { bg='#f0fdf4'; border='1px dashed #86efac'; textColor='#166534'; }
  return (
    <div onClick={() => onSelect(uid, choice)} style={{ display:'flex', alignItems:'flex-start', gap:'10px', padding:'10px 12px', borderRadius:'8px', background:bg, border, cursor:'pointer', transition:'all 0.15s', userSelect:'none' }}>
      <span style={{ minWidth:'22px', height:'22px', borderRadius:'50%', marginTop:'1px', background:isSelected?(choice.ok?'#10b981':'#ef4444'):'#f3f4f6', color:isSelected?'#fff':(CC[choice.num]?.text??'#374151'), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.78rem', fontWeight:'700', flexShrink:0 }}>{choice.num}</span>
  ...
  return (
    <PaywallGuard>
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'16px' }}>
        <div style={{ background:'#fff', borderRadius:'14px', padding:'24px', maxWidth:'380px', width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
          <h3 style={{ fontSize:'1.1rem', fontWeight:'800', marginBottom:'16px' }}>📊 개인화 진단 리포트</h3>
          <p style={{ fontSize:'0.85rem', color:'#6b7280', marginBottom:'12px' }}>이번 세션 오답 {wrongLog.length}개</p>
          {sorted.map(([pat,cnt]) => { const p=P[pat]; if(!p) return null; return (
          ...
          <button onClick={onClose} style={{ width:'100%', marginTop:'14px', padding:'10px', borderRadius:'8px', background:'#1f2937', color:'#fff', border:'none', fontWeight:'700', cursor:'pointer', fontSize:'0.9rem' }}>닫기</button>
        </div>
      </div>
    </PaywallGuard>
  );
