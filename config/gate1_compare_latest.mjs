...
if (!resultPath) {
  console.error(
    "Usage: node config/gate1_compare_latest.mjs <dryrun_results.json>",
  );
  process.exit(1);
}
...
const resultArr =
  results.results || results.samples || (Array.isArray(results) ? results : []);
...
for (const s of samples) {
  ...
  if (expectedVariants.length > 0) {
    ruleMatch = expectedVariants.some((v) => {
      const vv = new Set(v);
      return (
        vv.size === actualRules.size && [...vv].every((x) => actualRules.has(x))
      );
    });
  } else {
    ruleMatch =
...
const acceptable = rows.filter(
  (r) => r.status === "FULL_MATCH" || r.status === "ACCEPTABLE_RULE_DIVERGENCE",
).length;
const mismatchRows = rows.filter(
  (r) => r.status === "MISMATCH" || r.status === "MISSING_RESULT",
);
