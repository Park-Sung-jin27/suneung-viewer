...
(function(){
  ...
  const DEFAULT_QUESTION_IDS = ['long_partner', 'heart_closes', 'choice_standard'];
  ...
  const ELEMENT_COORDS = {
    '목': {
      label:'자라는 마음',
      scores:{ speed:72, attraction:66, expression:62, stability:54, recovery:58 },
      levels:{ speed:'빠르게 반응', attraction:'함께 자람', expression:'조심스런 제안', stability:'성장감', recovery:'대화로 조정' },
      teaser:'마음이 움직이면 먼저 가능성을 키워보는 쪽이에요. 다만 커지는 마음만큼 내 리듬도 같이 챙겨야 편합니다.'
    },
    '화': {
      label:'밝아지는 마음',
      scores:{ speed:84, attraction:78, expression:76, stability:48, recovery:64 },
      levels:{ speed:'빠르게 켜짐', attraction:'온기와 반응', expression:'표현 선명', stability:'기분 영향', recovery:'풀리면 빠름' },
      teaser:'마음이 켜지는 순간이 빠른 편이에요. 설렘이 살아날수록, 식은 뒤에도 지킬 태도를 같이 보는 게 좋습니다.'
    },
    '토': {
      label:'쌓이는 마음',
      scores:{ speed:46, attraction:50, expression:42, stability:82, recovery:48 },
      levels:{ speed:'천천히 확인', attraction:'익숙함 선호', expression:'말보다 행동', stability:'반복과 약속', recovery:'시간 필요' },
      teaser:'마음은 천천히 쌓이지만 한 번 편해진 자리는 오래 갑니다. 약속과 반복되는 태도가 가장 큰 기준이에요.'
    },
    '금': {
      label:'선명한 마음',
      scores:{ speed:58, attraction:62, expression:54, stability:78, recovery:44 },
      levels:{ speed:'선별 후 반응', attraction:'태도와 기준', expression:'짧고 분명', stability:'경계와 약속', recovery:'정리 후 대화' },
      teaser:'좋고 싫음의 기준이 비교적 선명한 편이에요. 말보다 태도가 맞는 사람에게 오래 마음이 갑니다.'
    },
    '수': {
      label:'깊어지는 마음',
      scores:{ speed:38, attraction:48, expression:34, stability:62, recovery:38 },
      levels:{ speed:'천천히 깊게', attraction:'뉘앙스와 깊이', expression:'정리 후 표현', stability:'정서적 안심', recovery:'혼자 정리' },
      teaser:'겉으로는 잔잔해 보여도 안에서는 오래 살피는 힘이 있어요. 재촉보다 마음이 움직일 시간이 필요합니다.'
    }
  };
  ...
  const QUESTION_BANK = [
    {
      ...
      answer: ({profile}) => profile.relation + ' 오래 갈 사람은 첫인상의 강함보다, 대화가 끝난 뒤 내 마음이 편안해지는 쪽에 가깝습니다. 풀 리포트에서는 오래 갈 상대 3유형과 피해야 할 관계 신호를 따로 나눠 보여드립니다.'
    },
    {
      id: 'heart_closes',
      label: '나는 관계에서 어떤 순간에 마음이 닫힐까?',
      answer: ({profile}) => profile.caution + ' 관계에서는 상대가 나를 재촉한다고 느낄 때 마음이 빨리 닫힐 수 있습니다. 이게 마음이 식은 신호인지, 부담이 커진 신호인지는 풀 리포트에서 따로 갈라 봐야 합니다.'
    },
    {
      id: 'date_or_focus',
      label: '소개팅을 해도 괜찮을까, 일에 집중할까?',
      answer: ({profile}) => profile.decision + ' 지금은 소개팅과 일 중 하나를 완전히 닫기보다, 에너지를 작게 나누는 쪽이 맞습니다. 풀 리포트에서는 이번 달에 넓혀도 되는 접점과 쉬어야 하는 접점을 구분해드립니다.'
    },
    {
      id: 'open_or_rest',
      label: '지금은 인연을 넓힐 때일까, 나를 정리할 때일까?',
      answer: ({profile}) => profile.self + ' 지금은 인연을 무작정 넓히기보다, 편안해지는 기준을 먼저 잡는 편이 좋습니다. 풀 리포트에서는 새 만남을 열어도 되는 타이밍과 잠시 쉬는 게 나은 타이밍을 나눠 봅니다.'
    },
    {
      id: 'work_drain',
      label: '내가 일에서 지치기 쉬운 패턴은?',
      answer: ({profile}) => profile.self + ' 일에서는 혼자 오래 붙잡고 정리하다가 지치는 패턴으로 나타날 수 있습니다. 풀 리포트에서는 어떤 환경에서 에너지가 새는지, 어떤 직무 방식이 덜 지치게 하는지까지 봅니다.'
    },
    {
      id: 'work_direction',
      label: '지금 나에게 맞는 일의 방향은 무엇일까?',
      answer: ({profile}) => profile.decision + ' 일의 방향은 직업명 하나보다, 내가 편하게 오래 쓰는 기능에서 먼저 보입니다. 풀 리포트에서는 이 기능을 실제 직무군과 연결해 더 구체적으로 좁혀드립니다.'
    },
    {
      id: 'money_leak',
      label: '돈이 새기 쉬운 습관은 무엇일까?',
      answer: ({profile}) => profile.caution + ' 돈에서는 기분을 달래기 위한 작은 지출이나, 고민을 미루기 위한 결제로 나타날 수 있습니다. 무료 미리보기에서는 수익을 예측하지 않고, 돈을 쓰게 되는 감정의 문턱만 가볍게 봅니다.'
    },
    {
      id: 'year_caution',
      label: '올해 내가 가장 조심해야 할 선택은?',
      answer: ({profile}) => profile.decision + ' 올해 조심할 선택은 외부의 기대가 내 판단보다 앞서는 순간입니다. 풀 리포트에서는 이 흐름이 관계, 일, 돈 중 어디에서 더 강하게 나타나는지 나눠 봅니다.'
    },
    {
      id: 'anxiety_reason',
      label: '요즘 불안이 커지는 이유는 무엇일까?',
      answer: ({profile}) => profile.self + ' 불안은 앞이 캄캄할 때만 커지지 않습니다. 내 마음의 박자와 현실의 요구가 어긋날 때도 커집니다. 풀 리포트에서는 불안 밑에 깔린 진짜 질문을 찾아 다음 선택 기준으로 바꿔드립니다.'
    },
    {
      id: 'choice_standard',
      label: '지금 선택 앞에서 먼저 봐야 할 기준은?',
      answer: ({profile}) => profile.decision + ' ' + profile.self + ' 선택이 커질수록 남들이 좋아 보이는 길보다, 내가 반복해서 지킬 수 있는 리듬인지 먼저 봐야 합니다. 풀 리포트에서는 A/B 선택지를 넣어 실제 권고 방향을 더 선명하게 뽑습니다.'
    },
    {
      id: 'weekly_action',
      label: '이번 주에 작게 해볼 행동은 무엇일까?',
      answer: ({profile}) => profile.caution + ' 이번 주에는 큰 변화보다 내 마음을 확인할 수 있는 장면 하나가 더 중요합니다. 풀 리포트에서는 지금 해볼 행동과 멈춰야 할 행동을 각각 한 줄로 정리해드립니다.'
    },
    {
      id: 'one_sentence',
      label: '내 기질을 한 문장으로 말하면?',
      answer: ({profile, pillarText}) => pillarText + '의 큰 결로 보면, 당신은 ' + profile.self + ' 한 문장으로 말하면, “내가 납득할 시간을 갖고 정리할 때 가장 단단해지는 사람”입니다. 풀 리포트에서는 이 기질이 연애, 일, 돈에서 각각 어떻게 다르게 드러나는지 펼쳐 봅니다.'
    }
  ];

  function answerCards(pillar, hour){
    const profile = ELEMENT_PREVIEW_PROFILES[pillar.element] || ELEMENT_PREVIEW_PROFILES['토'];
    const hourTone = hour?.tone || '태어난 시간을 비워 두면 시주의 세부 리듬은 줄이고, 일주의 큰 결만으로 읽습니다.';
    const pillarText = '일주 ' + pillar.stemKo + pillar.branchKo + '(' + pillar.stem + pillar.branch + ')';
  ...
  const ELEMENT_PROFILES = {
    '목': {
      ...
      relation: '같이 자라고 배워가는 느낌을 주는 사람에게 편안함을 느끼기 쉽습니다. 이미 완성된 사람보다, 대화 속에서 방향을 함께 넓혀가는 사람이 오래 남습니다.',
      caution: '이번 주에는 마음이 앞서서 약속을 크게 잡거나, 상대의 반응을 빨리 확인하고 싶어질 수 있습니다.',
      decision: '지금 선택 앞에서는 “이 선택이 나를 조금이라도 자라게 하는가”를 먼저 보세요.'
    },
    '화': {
      self: '마음이 켜질 때는 빠르게 밝아지고, 분위기와 반응을 섬세하게 읽는 결이 있습니다.',
      relation: '감정을 숨기지 않고 따뜻하게 표현해 주는 사람에게 편안함을 느끼기 쉽습니다. 단, 처음의 설렘보다 식은 뒤에도 예의를 지키는지가 더 중요합니다.',
      caution: '이번 주에는 말이 빨라지거나, 순간의 기분으로 답을 정하고 싶어질 수 있습니다.',
      decision: '지금 선택 앞에서는 “지금의 뜨거움이 지나도 내가 책임질 수 있는가”를 먼저 보세요.'
    },
    '토': {
      self: '천천히 쌓고 오래 지키는 힘이 있습니다. 쉽게 흔들리지 않지만, 마음을 정하기까지 시간이 필요합니다.',
      relation: '매일 비슷한 자리에서 꾸준히 보여주는 사람에게 편안함을 느끼기 쉽습니다. 화려한 말보다 약속을 지키는 태도, 급한 밀당보다 안정적인 리듬이 오래 갑니다.',
      caution: '이번 주에는 혼자 버티려 하거나, 마음이 정리되지 않았는데도 괜찮은 척할 수 있습니다.',
      decision: '지금 선택 앞에서는 “내가 오래 감당할 수 있는 구조인가”를 먼저 보세요.'
    },
    '금': {
      self: '기준이 분명하고 애매한 것을 오래 두기 어려운 결이 있습니다. 정리가 되면 빠르고 단단합니다.',
      relation: '말과 행동이 일치하는 사람에게 편안함을 느끼기 쉽습니다. 호감 표현이 크지 않아도 태도가 선명하고, 약속과 경계를 존중하는 사람이 오래 갑니다.',
      caution: '이번 주에는 마음을 보호하려고 말을 너무 짧게 하거나, 상대의 부족한 부분을 먼저 재단할 수 있습니다.',
      decision: '지금 선택 앞에서는 “내 기준을 지키면서도 관계를 너무 빨리 잘라내고 있지는 않은가”를 먼저 보세요.'
    },
    '수': {
      self: '흐름을 읽고 깊이 생각하는 힘이 있습니다. 겉으로는 잔잔해 보여도 안에서는 감정과 가능성을 오래 살핍니다.',
      relation: '말하지 않은 뉘앙스까지 조심스럽게 읽어 주는 사람에게 편안함을 느끼기 쉽습니다. 재촉하지 않고, 성급한 결론 대신 마음이 움직일 시간을 주는 사람이 오래 남습니다.',
      caution: '이번 주에는 생각이 깊어지는 만큼 결정을 미루거나, 마음을 말하지 않은 채 상대가 알아주길 바랄 수 있습니다.',
      decision: '지금 선택 앞에서는 “이 선택이 내 마음을 더 흐리게 하는가, 아니면 맑게 하는가”를 먼저 보세요.'
    }
  };
  ...
  function axisBasis(key, pillar, hour){
    const hourText = hour ? '시주는 ' + hour.branchKo + '시 리듬까지 함께 봅니다.' : '생시가 없어 일주의 큰 결로만 봅니다.';
    const map = {
  ...
  function renderMiniRadar(profiles){
    ...
    const cx = 82, cy = 82, radius = 52;
    const rings = [33, 66, 100].map((level) => (
      '<polygon points="' + axes.map((_, i) => {
        const p = coordPoint(i, axes.length, level, radius, cx, cy);
        return p.x.toFixed(1) + ',' + p.y.toFixed(1);
      }).join(' ') + '" fill="none" stroke="rgba(216,161,93,.26)" stroke-width="1"/>'
    )).join('');
    const spokes = axes.map((_, i) => {
      const p = coordPoint(i, axes.length, 100, radius, cx, cy);
      return '<line x1="' + cx + '" y1="' + cy + '" x2="' + p.x.toFixed(1) + '" y2="' + p.y.toFixed(1) + '" stroke="rgba(216,161,93,.2)" stroke-width="1"/>';
    }).join('');
    const labels = axes.map((axis, i) => {
      const p = coordPoint(i, axes.length, 124, radius, cx, cy);
      return '<text x="' + p.x.toFixed(1) + '" y="' + (p.y + 3).toFixed(1) + '" text-anchor="middle">' + axis.label.replace(' ', '\n') + '</text>';
    }).join('');
    const colors = ['#f1c987', '#a9d7bf'];
    const polys = items.map((profile, profileIndex) => {
      const points = profile.axes.map((axis, i) => {
        const p = coordPoint(i, profile.axes.length, axis.score, radius, cx, cy);
        return p.x.toFixed(1) + ',' + p.y.toFixed(1);
      }).join(' ');
      const color = colors[profileIndex % colors.length];
      return '<polygon points="' + points + '" fill="' + color + '" fill-opacity=".16" stroke="' + color + '" stroke-width="2.2"/>';
    }).join('');
    const legend = items.map((profile, i) => '<span><i style="background:' + colors[i % colors.length] + '"></i>' + escapeHtml(profile.displayName || profile.name || '나') + '</span>').join('');
    const axisCards = axes.map((axis, i) => {
      const lines = items.map((profile) => {
        const found = profile.axes[i] || {};
        return '<p><b>' + escapeHtml(profile.displayName || '나') + '</b> ' + escapeHtml(found.level || '중간') + '</p>';
      }).join('');
      return '<div><b>' + escapeHtml(axis.label) + '</b>' + lines + '</div>';
    }).join('');
    return '<div class="fp-radar-head"><b>무료 요약 좌표</b><span>일주·시주로 보는 간단한 체감 단계입니다</span></div>' +
      '<div class="fp-radar-layout"><div><svg viewBox="0 0 164 164" role="img" aria-label="무료 미리보기 미니 레이더">' +
      rings + spokes + polys + labels + '<circle cx="' + cx + '" cy="' + cy + '" r="2.8" fill="#f1c987"/></svg><div class="fp-radar-legend">' + legend + '</div></div>' +
      '<div class="fp-radar-list">' + axisCards + '</div></div>';
  }
  ...
  function trackPreviewEvent(name, detail){
    const payload = { name, detail: detail || {}, at: new Date().toISOString() };
    try {
      window.dispatchEvent(new CustomEvent('jippi-preview-event', { detail: payload }));
  ...
  function pillarLabel(pillar){
    return pillar.stemKo + pillar.branchKo + '(' + pillar.stem + pillar.branch + ')';
  }
  ...
  function readPreviewPerson(opts){
    ...
    if(!parsed) throw new Error(opts.label + ' 생년월일은 1999-09-09처럼 4자리 연도로 입력해 주세요.');
    if(dateInput) dateInput.value = parsed.formatted;
    if(parsed.y < 1900 || parsed.y > 2100) throw new Error('1900~2100년 사이만 지원합니다.');

    const cal = ($(opts.calendarId) || {}).value || 'solar';
    let solar = { y: parsed.y, m: parsed.m, d: parsed.d };
    if(cal === 'lunar') solar = lunarToSolar(parsed.y, parsed.m, parsed.d, false);

    const pillar = dayPillar(solar.y, solar.m, solar.d);
    const hour = hourPillar(STEMS.indexOf(pillar.stem), (($(opts.timeId) || {}).value || ''));
    const coords = profileCoordinates(pillar, hour);
    coords.displayName = opts.displayName || opts.label;
    return { pillar, hour, coords, label: opts.label, displayName: opts.displayName || opts.label };
  }
  ...
  function setPreviewMode(mode){
    ...
    if(soloBtn){
      ...
      soloBtn.setAttribute('aria-pressed', previewMode === 'solo' ? 'true' : 'false');
    }
    if(pairBtn){
      pairBtn.classList.toggle('is-active', previewMode === 'pair');
      pairBtn.setAttribute('aria-pressed', previewMode === 'pair' ? 'true' : 'false');
    }
    ...
    const btn = $('fpRunBtn');
    if(btn) btn.textContent = previewMode === 'pair' ? '둘이 보기 미리보기 →' : '무료 미리보기 →';
  }

  function pairTeaser(a, b){
    const speedA = a.coords.axes.find((axis) => axis.key === 'speed')?.score || 56;
    const speedB = b.coords.axes.find((axis) => axis.key === 'speed')?.score || 56;
    const stabilityA = a.coords.axes.find((axis) => axis.key === 'stability')?.score || 56;
    const stabilityB = b.coords.axes.find((axis) => axis.key === 'stability')?.score || 56;
    ...
    if(speedGap >= 28){
      return {
        ...
        friction: '시간이 지나면 빠른 쪽은 답답해지고, 신중한 쪽은 밀린다고 느끼기 쉽습니다.',
    ...
    if(bothStable){
      return {
        ...
        friction: '확실한 약속 없이 분위기로만 흘러가면 둘 다 마음을 덜 열 수 있습니다.',
    ...
    return {
      ...
      friction: '작은 서운함을 말로 맞추지 않으면, 둘 사이의 온도가 천천히 낮아질 수 있어요.',
  ...
  function renderPreviewResult(payload){
    ...
    [['fpQ1Title','fpQ1Text'], ['fpQ2Title','fpQ2Text'], ['fpQ3Title','fpQ3Text']].forEach((ids, index) => {
  ...
  function drawShareImage(){
    ...
    wrapCanvasText(ctx, lastPreview.shareTitle || lastPreview.teaserTitle || '무료 미리보기', 86, 230, 908, 76, 3);
    ...
    const cx = 540, cy = 835, radius = 210;
    ctx.strokeStyle = 'rgba(241,201,135,.28)';
    ctx.lineWidth = 2;
    [0.33, 0.66, 1].forEach((r) => {
      ctx.beginPath();
      COORDINATE_AXES.forEach((_, i) => {
        const p = coordPoint(i, COORDINATE_AXES.length, r * 100, radius, cx, cy);
    ...
    (lastPreview.profiles || []).slice(0, 2).forEach((profile, profileIndex) => {
      ...
      ctx.closePath();
      ctx.fillStyle = profileIndex ? 'rgba(169,215,191,.18)' : 'rgba(241,201,135,.18)';
    ...
    ctx.fillText('입력 정보는 저장되지 않고 브라우저 안에서만 계산돼요', 86, 1230);
    ctx.fillStyle = '#fff8ed';
  ...
  async function sharePreview(){
    ...
    const text = (lastPreview.shareTitle || lastPreview.teaserTitle || 'JIPPI 무료 미리보기') + '\n' + (lastPreview.teaserText || '') + '\n' + url;
    trackPreviewEvent('free_preview_share_click', { mode: lastPreview.mode });
    if(navigator.share){
      try {
        await navigator.share({ title:'JIPPI 무료 미리보기', text, url });
        trackPreviewEvent('free_preview_share_success', { mode: lastPreview.mode });
        return;
      } catch(e) {
  ...
  function renderQuestionChoices(){
    ...
    QUESTION_BANK.forEach((question) => {
      ...
      btn.type = 'button';
      btn.className = 'fp-choice' + (isSelected ? ' is-selected' : '') + (isDisabled ? ' is-disabled' : '');
  ...
  function run(){
    try {
      ...
      if(previewMode === 'pair'){
        ...
        renderPreviewResult({
          ...
          cards:[
            ...
            { title:'지금 피할 행동', text:teaser.avoid + ' 풀 리포트에서는 화해 순서와 첫 문장, 30일 운영표까지 열립니다.' }
          ]
        });
        return;
      }
      ...
      const teaser = {
        ...
        text: me.coords.teaser || (ELEMENT_PROFILES[me.pillar.element] || ELEMENT_PROFILES['토']).self
      };
      renderPreviewResult({
        mode:'solo',
  ...
  function init(){
    ...
    const soloBtn = $('fpModeSolo');
    const pairBtn = $('fpModePair');
    if(soloBtn) soloBtn.addEventListener('click', () => setPreviewMode('solo'));
    if(pairBtn) pairBtn.addEventListener('click', () => setPreviewMode('pair'));
