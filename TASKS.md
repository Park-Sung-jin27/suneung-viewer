# TASKS.md — 실행 엔진

> 갱신: 2026-04-15
> 항상 3개만 유지. 완료되면 아래 DONE으로 이동.

---

## 🔥 ACTIVE TASKS

**[B-1] 해설 재생성**
- 담당: Code B
- 완료 기준: `reanalyze_positive.mjs all` 실행 후 반전/빈 해설 0건
- 상태: 실행 중 (worktree 아닌 main repo에서 실행할 것)
- 명령어: `cd C:/Users/downf/suneung-viewer && node pipeline/reanalyze_positive.mjs all`

**[B-2] quality_gate 3단계 필터 개선**
- 담당: Code B
- 완료 기준:
  - CRITICAL / WARNING / IGNORE 3단계 분류 작동
  - reanalyze 결과 improved / still_bad / needs_human 분류
  - needs_human만 출력 (전체 검수 금지)
- 상태: B-1 완료 후 진행
- 원칙: 복잡한 자동화 추가 금지. 현재 구조 유지하면서 필터만 추가.

**[A-1] 랜딩페이지 "이렇게 공부하세요" 섹션 추가**
- 담당: Code A
- 완료 기준: 랜딩페이지에 3단계 활용법 섹션 노출
- 상태: 진행
- 내용:
  ```
  1단계: 먼저 풀어보세요
    시간 재지 말고, 평소대로 풀어보세요.

  2단계: 형광펜으로 근거를 확인하세요
    모든 선지에 지문 근거가 연결되어 있습니다.
    내가 고른 답의 근거가 실제로 지문에 있는지 확인해보세요.

  3단계: 해설로 완성하세요
    왜 맞고 왜 틀렸는지, 지문 어디에서 판단해야 했는지
    확인하면 같은 실수를 반복하지 않습니다.
  ```
- 원칙: 기능 추가 아님. 텍스트/UI 섹션만. 3초 안에 이해 가능하게.

---

## 📌 2단계 진입 조건

아래 조건 만족 시 다음 단계로:
- CRITICAL = 0
- WARNING 자동 처리 안정
- needs_human 20건 이하

---

## ❌ DO NOT TOUCH (이번 주)

- 전체 시험 확장 (2014~2021)
- UI 리디자인
- 신규 기능 추가 (근거 찾기 인터랙션 등)
- pat_unclassifiable 190건 (다음 주)
- 전체 검수 / 무작위 샘플 확인
- 복잡한 자동화 추가

---

## ✅ DONE

- [x] DEAD cs_ids 138건 → 0건 (2022·2023수능)
- [x] AI노출/ID잔재 11건 → 0건
- [x] step3 프롬프트 4단계 원칙 반영
- [x] step4 AUTO_EMPTY_PATS 설계
- [x] 부실해설 87건 재생성 (2022~2026수능)
- [x] quality_gate --fix 546건
- [x] l2023b Q22 전체 교체
- [x] 프로젝트 파일 구조 정리
- [x] CLAUDE.md 통합 재작성
- [x] 2025수능 해설 ID 잔재 13건 → 0건
- [x] 루트/src 레거시 파일 삭제 커밋

---

## 완료 보고 형식 (Code A/B 필수)

```
[완료 보고]
작업:
완료 기준 충족: Y/N
결과:
문제:
```
